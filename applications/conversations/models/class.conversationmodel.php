    ...
    public function counts($column, $from = false, $to = false, $max = false)
    {
        ...
        switch ($column) {
            ...
            case "CountParticipants":
                $this->Database->query(
                    DBAModel::getCountSQL("count", "Conversation", "UserConversation", "CountParticipants", "UserID")
                );
                break;
            case "FirstMessageID":
                $this->Database->query(
                    DBAModel::getCountSQL("min", "Conversation", "ConversationMessage", $column, "MessageID")
                );
                break;
            case "LastMessageID":
                $this->Database->query(
                    DBAModel::getCountSQL("max", "Conversation", "ConversationMessage", $column, "MessageID")
                );
                break;
            case "DateUpdated":
                $this->Database->query(
                    DBAModel::getCountSQL("max", "Conversation", "ConversationMessage", $column, "DateInserted")
                );
                break;
            case "UpdateUserID":
                $this->SQL
