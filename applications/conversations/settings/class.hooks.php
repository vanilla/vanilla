...
class ConversationsHooks implements Gdn_IPlugin
{
    ...
    {
        $dic->rule(\Vanilla\Menu\CounterModel::class)->addCall("addProvider", [
            new Reference(ConversationCounterProvider::class),
        ]);
    ...
    public function profileController_addProfileTabs_handler($sender)
    {
        if (Gdn::session()->isValid()) {
            ...
            if (Gdn::session()->UserID != $sender->User->UserID) {
                ...
                if (
                    c("Conversations.Moderation.Allow", false) &&
                    Gdn::session()->checkPermission("Conversations.Moderation.Manage")
                ) {
    ...
    {
        $sender->addDefault(RoleModel::TYPE_MEMBER, ["Conversations.Conversations.Add" => 1]);
        $sender->addDefault(RoleModel::TYPE_MODERATOR, ["Conversations.Conversations.Add" => 1]);
        $sender->addDefault(RoleModel::TYPE_ADMINISTRATOR, ["Conversations.Conversations.Add" => 1]);
    }
