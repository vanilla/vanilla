...
use Gdn;
use LogModel;
use Vanilla\AutomationRules\Actions\ExternalAction;
        ...
        foreach ($this->triggerTypes as $triggerType) {
            ...
            if (empty($activeAutomationRules)) {
                ...
                foreach ($activeAutomationRules as $automationRule) {
                    ...
                    // If post does not have Tag from the trigger, we don't need to process this event.
                    if (!empty($automationRule["triggerValue"]["tagID"])) {
                        $tagModel = Gdn::getContainer()->get(\TagModel::class);
                        $tags = $tagModel->getTagIDsForDiscussionIDs([$args["RecordID"]]);
                        if (
                            empty(
                                array_intersect(
                                    $automationRule["triggerValue"]["tagID"],
                                    $tags[$args["RecordID"]] ?? []
                                )
                            )
                        ) {
                    ...
                    // If post does not have Category from the trigger, we don't need to process this event.
                    $recordType = $args["Record"]["CategoryID"];
                    if (
                        !empty($automationRule["triggerValue"]["categoryID"]) &&
                        !in_array($recordType, $automationRule["triggerValue"]["categoryID"])
                    ) {
                        continue;
                    }
                    ...
                    if (is_a($action, ExternalAction::class)) {
                        $action->executeLongRunner([], $args["Record"]);
                    } else {
