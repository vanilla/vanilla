...
use Vanilla\Dashboard\Events\DiscussionStatusEvent;
use Vanilla\Dashboard\Events\IdeaStatusEvent;
use Vanilla\Events\EventAction;
use Vanilla\Exception\Database\NoResultsException;
use Vanilla\Http\InternalClient;

/**
 * Handle record status events
 */
class RecordStatusEventHandler {
     ...
     *
     * @param IdeaStatusEvent $event Event to handle
     */
    public function handleIdeaStatusEvent(IdeaStatusEvent $event): void {
     ...
     *
     * @param IdeaStatusEvent $event Idea Status Added Event to handle
     */
    private function handleIdeaStatusAddition(IdeaStatusEvent $event): void {
        ...
        if (empty($ideaStatus['recordStatusID'])) {
            ...
            if ($response->isSuccessful()) {
                ...
                $addEvent = new DiscussionStatusEvent(
                    EventAction::ADD,
                    $discussionStatusID,
                    $discussionStatus,
                    $event->getUserID(),
     ...
     *
     * @param IdeaStatusEvent $event Idea Status Update event to handle
     */
    private function handleIdeaStatusUpdate(IdeaStatusEvent $event): void {
     ...
     *
     * @param IdeaStatusEvent $event Idea Status Delete event to handle
     */
    private function handleIdeaStatusDelete(IdeaStatusEvent $event): void {
        if (is_numeric($event->getForeignID())) {
            $path = "/api/v2/discussions/statuses/{$event->getForeignID()}";
            try {
                $this->apiClient->setUserID($event->getUserID());
