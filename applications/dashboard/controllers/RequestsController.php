...
class RequestsController extends \Gdn_Controller
{
     ...
     */
    public function __construct(RoleRequestsApiController $roleRequests, \RoleModel $roleModel)
    {
        parent::__construct();
    ...
    public function roleApplications($role): void
    {
        ...
        $role = $this->roleModel
            ->getWhere([is_numeric($role) ? "RoleID" : "Name" => $role])
        ...
        if (in_array($roleID, Gdn::userModel()->getRoleIDs(Gdn::session()->UserID))) {
            ...
            $canReApply =
                !empty($meta["attributes"]["allowReapply"]) &&
                $application["status"] === RoleRequestModel::STATUS_DENIED;
