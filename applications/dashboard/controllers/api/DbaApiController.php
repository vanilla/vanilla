    ...
    public function patch_counts(array $body = []): Data
    {
        ...
        $in = Schema::parse([
            "tables:a" => [
                "items" => [
                    "type" => "string",
                    "enum" => ["discussion", "conversation", "category", "comment"],
                ],
            ],
        ]);
        ...
        $finalResult = new Data(["counts" => $result], $result->getStatus());
        $models = array_intersect_key(UserMentionsModel::INDEXABLE_RECORDS, array_flip($tables));
        if (count($models) > 0) {
            $finalResult->addData(
                $this->longRunner->runApi(
