...
class DigestApiController extends Controller
{
     ...
     */
    public function get_deliveryDates(): Data
    {
        ...
        $recentScheduledDates = $this->digestModel->getRecentWeeklyDigestScheduledDates(5);
        if (!empty($recentScheduledDates)) {
            if (DateTimeFormatter::dateTimeToTimeStamp($recentScheduledDates[0]) > $now->getTimestamp()) {
        ...
        if ($isDigestEnabled) {
            $data["upcoming"] = $this->getFutureScheduledDates();
        }
     ...
     * @return array
     */
    private function getFutureScheduledDates(): array
    {
        $upcomingSchedules = [];
        $nextScheduledDay = "";
        $maxIterations = 5;
        for ($i = 0; $i < $maxIterations; $i++) {
            $nextScheduledDay =
                $i == 0 ? $this->scheduleWeeklyDigestJob->getNextScheduledDate() : $nextScheduledDay->modify("+1 week");
