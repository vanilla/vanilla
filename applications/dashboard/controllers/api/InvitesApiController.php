...
class InvitesApiController extends AbstractApiController
{
    ...
    public function index(array $query)
    {
        ...
        $in = $this->schema(
            ...
            "in"
        )->setDescription("Get a list of invites sent by the current user.");
        $out = $this->schema(
            [
                ":a" => $this->fullSchema(),
            ],
        ...
        $rows = $this->invitationModel->getByUserID($userID, "", $limit, $offset, false)->resultArray();

        // Expand associated rows.
        $this->userModel->expandUsers($rows, $this->resolveExpandFields($query, ["acceptedUser" => "acceptedUserID"]));
        ...
        $paging = ApiUtils::numberedPagerInfo(
            $this->invitationModel->getCount(["InsertUserID" => $userID]),
            "/api/v2/invites",
            $query,
            $in
        );
