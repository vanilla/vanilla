...
class ProfileFieldsApiController extends \AbstractApiController
{
    ...
    public function index(array $query = []): Data
    {
        ...
        $rows = $this->profileFieldModel->select($where, ["orderFields" => "sort", "orderDirection" => "asc"]);

        $out = $this->schema($this->profileFieldModel->getOutputSchema(), "out");
    ...
    public function post(array $body): Data
    {
        ...
        $in = $this->schema($this->profileFieldModel->getFullSchema());
    ...
    public function patch(string $apiName, array $body): Data
    {
        ...
        $in = $this->schema($this->profileFieldModel->getPatchSchema());
        $body = $in->validate($body);

        $body = ArrayUtils::mergeRecursive($profileField, $body);
        $body = $this->profileFieldModel->getFullSchema()->validate($body);
    ...
    protected function getProfileFieldByApiName(string $apiName): array
    {
        try {
            ...
            $outputSchema = $this->schema($this->profileFieldModel->getOutputSchema(), "out");
