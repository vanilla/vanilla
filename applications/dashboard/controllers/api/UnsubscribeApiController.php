    ...
    public function post(string $token): array
    {
        ...
        if (count($result) == 1) {
            if (isset($result["FollowedCategory"])) {
                ...
                $result[0]["enabled"] = "0";
                Gdn::userModel()->savePreference($this->activityInfo["notifyUserID"], $result[0]["preference"], "0");
            }
        }
        $follow = [];
        if (isset($result["FollowedCategory"])) {
    ...
    public function post_resubscribe(string $token): array
    {
        ...
        if (count($result) == 1) {
            if (isset($result["FollowedCategory"])) {
                ...
                $result[0]["enabled"] = "1";
                Gdn::userModel()->savePreference($this->activityInfo["notifyUserID"], $result[0]["preference"], "1");
            }
        }
        $follow = [];
        if (isset($result["FollowedCategory"])) {
