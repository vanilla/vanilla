        ...
        if (count($result) == 1) {
            if (isset($result["FollowedCategory"])) {
                if ($result["FollowedCategory"]) {
                    if ($result["FollowedCategory"]["preference"] == "follow") {
                        $categoryModel = Gdn::getContainer()->get(CategoryModel::class);
                        $categoryModel->setPreferences(
                            $this->user["UserID"],
                            $result["FollowedCategory"]["categoryID"],
                            [
                                CategoryModel::stripCategoryPreferenceKey(CategoryModel::PREFERENCE_FOLLOW) => "0",
                            ]
                        );
                    } else {
    ...
    public function post_resubscribe(string $token): array
    {
        ...
        if (count($result) == 1) {
            if (isset($result["FollowedCategory"])) {
                ...
                if ($result["FollowedCategory"]["preference"] == "follow") {
                    $categoryModel = Gdn::getContainer()->get(CategoryModel::class);
                    $categoryModel->setPreferences($this->user["UserID"], $result["FollowedCategory"]["categoryID"], [
                        CategoryModel::stripCategoryPreferenceKey(CategoryModel::PREFERENCE_FOLLOW) => $result[
                            "FollowedCategory"
                        ]["enabled"],
                    ]);
                } else {
        ...
        if ($followCategory != null && $follow != null && $follow["enabled"] != $followCategory["enabled"]) {
            ...
            if ($followCategory["preference"] == "follow") {
                ...
                $categoryModel->setPreferences($this->user["UserID"], $followCategory["categoryID"], [
                    CategoryModel::stripCategoryPreferenceKey(CategoryModel::PREFERENCE_FOLLOW) => $followCategory[
                        "enabled"
                    ],
                ]);
            } else {
            ...
            if (str_starts_with($reason, "FollowedCategory:")) {
                ...
                if ($enabled === null || val($reason, $this->metaPrefs, 0) == $enabled) {
                    $result[$categoryInfo[0]] = [
                        ...
                        "preference" =>
                            $categoryInfo[2] == "newFollowedCategory"
                                ? "follow"
                                : "Preferences.Email.$categoryInfo[2].{$category["CategoryID"]}",
                        "name" => $category["Name"],
                        "enabled" => $enabled,
                    ];
                }
