...
class UsersApiController extends AbstractApiController
{
    ...
    public function get(int $id, array $query): Data
    {
        ...
        if (ModelUtils::isExpandOption(ModelUtils::EXPAND_CRAWL, $expand)) {
            $userProfileFields = $this->profileFieldModel->getUserProfileFields($id);
            if (!empty($userProfileFields)) {
                $row["profileFields"] = $userProfileFields;
            }
        }
    ...
    {
        if (($result = $this->tryWithUserSearch($query)) instanceof Data) {
            return $result;
        }
        ...
        if (isset($query["extended"])) {
            $where["extended"] = $query["extended"];
        }
        ...
        if (ModelUtils::isExpandOption(ModelUtils::EXPAND_CRAWL, $expand)) {
            $this->userModel->joinProfileFields($rows);
        }
    ...
    {
        $searchOnlyFilters = $this->filterNonEmptyValues($query, ["name", "email", "query", "ipAddresses"]);
        if (empty($searchOnlyFilters)) {
            return null;
        }

        // Need to initialize search service at runtime because of cyclical dependency.
        ...
        if (is_null($driver->getSearchTypeByType(UserSearchType::TYPE))) {
            throw new ClientException(
                "The following filters require the user search type which is not enabled: " .
                    implode(", ", array_keys($searchOnlyFilters))
            );
        }

        foreach (["dirtyRecords", "userID"] as $incompatibleFilter) {
            if (isset($query[$incompatibleFilter])) {
                throw new ClientException(
                    "$incompatibleFilter cannot be combined with the following filters: " .
                        implode(", ", array_keys($searchOnlyFilters))
                );
            }
        }
        ...
        $query["recordTypes"] = [UserSearchType::TYPE];
        if (isset($query["roleID"])) {
            $query["roleIDs"] = [$query["roleID"]];
        }
        $originalQuery = $query;
        $query = $in->validate($query);
        [$offset, $limit] = offsetLimit("p{$query["page"]}", $query["limit"]);
        $searchResults = $driver->search($query, new SearchOptions($offset, $limit, true));
        ...
        $finalQuery = ["userID" => $userIDs];
        if (isset($originalQuery["expand"])) {
            $finalQuery["expand"] = $originalQuery["expand"];
        }
        $rows = $this->index($finalQuery)->getData();
        $paging = ApiUtils::numberedPagerInfo($searchResults->getTotalCount(), "/api/v2/users", $query, $in);
        $meta = Pagination::tryCursorPagination($paging, $query, $rows, "userID") + ["api-allow" => ["email"]];
        return new Data($rows, $meta);
    }
