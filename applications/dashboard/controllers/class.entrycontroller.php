...
class EntryController extends Gdn_Controller implements LoggerAwareInterface
{
    ...
    public function connect($method)
    {
        ...
        if ($userID) {
            ...
            if (($emailVerified || ($emailVerified === null && $autoConnect)) && count($existingUsers) > 0) {
                if ($isPostBack && $this->Form->getFormValue("ConnectName")) {
     ...
     */
    public function generateFormCustomProfileFields()
    {
        ...
        foreach ($profileFields as $key => $field) {
            ...
            if ($fieldIsRequiredOrOptional && !$isTokenInput) {
                ...
                if ($formType == "CheckBox") {
                    echo wrap($this->Form->{$formType}($name, $fieldLabel), "li");
                } else {
                    echo wrap(
