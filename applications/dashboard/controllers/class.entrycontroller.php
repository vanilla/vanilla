...
class EntryController extends Gdn_Controller implements LoggerAwareInterface
{
    ...
    public function addCredentialErrorToForm($translationCode)
    {
        if (c("Garden.PrivateCommunity", false)) {
            $this->Form->addError($this->getCredentialErrorString());
        } else {
    ...
    public function connect($method)
    {
        ...
        try {
            ...
            if (empty($stashID)) {
                //check if we have any stash sessions provided by one of the connection providers
                $stashID = $this->Form->getValue("StashID", null);
            }
        ...
        if ($userID) {
            ...
            if ($isPostBack && $this->Form->getFormValue("ConnectName")) {
                ...
                $searchName = $this->Form->getFormValue("Name", $this->Form->getFormValue("ConnectName"));
            }
            ...
            $haveName = $this->Form->getFormValue("Name", $this->Form->getFormValue("ConnectName"));
            ...
            if ($didNotPickUser && $haveName && $emailValid && $noMatches && !$this->hasRegistrationCustomFields()) {
                ...
                // If registration method is set to approval, and user role has been assigned through sso follow basic registration
                if ($saveRolesRegister && $registrationMethod == "approval" && !empty($user["RoleID"])) {
                    $registerOptions["Method"] = "basic";
                }
                $user["ProfileFields"] = $this->processNonVisibleFields();
        ...
        if ($isPostBack) {
            ...
            if (isset($user) && $user) {
                ...
                $user["Password"] = randomString(16); // Required field.
                $user["HashMethod"] = "Random";
                $user["ProfileFields"] = $this->reformatProfileFields($user["Profile"] ?? []);
            ...
            */
            ...
            if ($notVisible && in_array($profileField["formType"], ["dropdown", "tokens"])) {
                ...
                $dropdownOptions = is_array($profileField["dropdownOptions"])
                    ...
                    : json_decode($profileField["dropdownOptions"], true);
    ...
    private function registerApproval()
    {
        ...
        if ($this->Form->isPostBack()) {
            ...
            try {
                ...
                unset($values["Roles"]);
                $values["ProfileFields"] = $this->reformatProfileFields($values["Profile"] ?? []);
    ...
    public function generateFormCustomProfileFields(?array $wrapperOptions = null, bool $connectPage = false)
    {
        ...
        foreach ($profileFields as $key => $field) {
            ...
            } elseif ($formType == "datepickerInputReact") {
                $result .= wrap($this->Form->{$formType}($name, $field["label"], $field["description"]), "li", [
                    "class" => "form-group",
                ]);
            } else {
    ...
    /**
     * Get all non-empty profile field values with token values converted to the correct format.
     * @return array
     */
    private function reformatProfileFields(array $formProfileFields): array
    {
        $profileFields = $this->profileFieldModel->getProfileFields(["enabled" => 1]) ?? [];
    ...
    /**
     * Extract hidden or internal profile fields.
     *
     * @return array Hidden profile fields if they exist
     */
    private function processNonVisibleFields(): array
    {
        $profileFields = $this->Data["nonVisibleFields"] ?? [];
        $profileErrors = [];

        // If there are existing profile fields.
    ...
    /**
     * Give credential error message
     *
     * @return string
     */

    private function getCredentialErrorString(): string
    {
        return "@" .
