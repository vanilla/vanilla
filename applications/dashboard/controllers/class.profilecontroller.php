    ...
    public function preferences($userReference = "", $username = "", $userID = "")
    {
        ...
        foreach ($this->Preferences as $prefGroup => $prefs) {
            foreach ($prefs as $pref => $desc) {
                ...
                $defaultValue = Gdn::config("Preferences." . $pref, "0");
                if ($location == "Meta") {
                    $currentPrefs[$pref] = val($pref, $metaPrefs, $defaultValue);
                } else {
                    $currentPrefs[$pref] = val($pref, $userPrefs, $defaultValue);
                }
        ...
        if ($this->Form->authenticatedPostBack()) {
            ...
            // For unauthenticated post populate form values with current preferences
            $this->Form->setFormValue($currentPrefs);
        } else {
