            ...
            try {
                ...
                switch ($Exception->getCode()) {
                    ...
                    case 1045:
                        $this->Form->addError(
                            t(
                                "Failed to connect to the database with the username and password you entered. Did you mistype them? The database reported: <code>%s</code>"
                            ),
                            strip_tags($Exception->getMessage())
                        );
                        break;
                    case 1049:
                        $this->Form->addError(
                            t(
                                "It appears as though the database you specified does not exist yet. Have you created it yet? Did you mistype the name? The database reported: <code>%s</code>"
                            ),
                            strip_tags($Exception->getMessage())
                        );
                        break;
                    case 2005:
                        $this->Form->addError(
                            t(
                                "Are you sure you've entered the correct database host name? Maybe you mistyped it? The database reported: <code>%s</code>"
                            ),
                            strip_tags($Exception->getMessage())
                        );
                        break;
    ...
    private function _checkPrerequisites()
    {
        ...
        if (version_compare(phpversion(), ENVIRONMENT_PHP_VERSION) < 0) {
            $this->Form->addError(
                sprintf(
                    t(
                        'You are running PHP version %1$s. Vanilla requires PHP %2$s or greater. You must upgrade PHP before you can continue.'
                    ),
                    phpversion(),
                    ENVIRONMENT_PHP_VERSION
                )
            );
        }
        ...
        $requiredPHPModules = [
            "pdo" =>
                'You must have the <a href="' .
                $phpManual .
                'pdo.installation.php">PDO module</a> enabled in PHP in order for Vanilla to connect to your database.',
            "pdo_mysql" =>
                'You must have the <a href="' .
                $phpManual .
                'ref.pdo-mysql.php">MySQL driver for PDO</a> enabled in order for Vanilla to connect to your database.',
            "mysqlnd" =>
                'You must have the <a href="' .
                $phpManual .
                'mysqlnd.install.php">MySQL Native Driver</a> enabled in order for Vanilla to connect to your database.',
            "curl" =>
                'You must have the <a href="' . $phpManual . 'curl.installation.php">cURL module</a> enabled in PHP.',
            "gd" =>
                'You must have the <a href="' .
                $phpManual .
                'image.installation.php">gd module</a> enabled in PHP for processing images.',
            "mbstring" =>
                'You must have the <a href="' .
                $phpManual .
                'mbstring.installation.php">mbstring module</a> enabled in PHP.',
            "intl" =>
                'You must have the <a href="' .
                $phpManual .
                'intl.installation.php">Internationalization Functions</a> enabled in PHP.',
        ];
            ...
            if (file_exists($configFile)) {
                ...
                if (!is_writeable(dirname($configFile))) {
                    $this->Form->addError(
                        sprintf(
                            t(
                                "Your configuration file cannot be created. PHP needs to be able to create this file: <code>%s</code>"
                            ),
                            $configFile
                        )
                    );
