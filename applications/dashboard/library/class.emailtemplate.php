...
class EmailTemplate extends Gdn_Pluggable implements Gdn_IEmailTemplate
{
    ...
    public function __construct($message = "", $title = "", $lead = "", $view = "email-basic")
    {
        ...
        $this->setContainerBackgroundColor(
            c("Garden.EmailTemplate.ContainerBackgroundColor", self::DEFAULT_CONTAINER_BACKGROUND_COLOR)
        );
        $this->setDefaultButtonBackgroundColor(
            c("Garden.EmailTemplate.ButtonBackgroundColor", self::DEFAULT_BUTTON_BACKGROUND_COLOR)
        );
    ...
    public function modernStylesSchema(): Schema
    {
        return Schema::parse([
            "logoUrl:s?",
            "textColor:s?",
    ...
    /**
     * Apply an array of template styles in the APIv2 format. See /api/v2/emails.
     *
     * @param array $params
     * @return void
     */
    public function applyModernStyles(array $params)
    {
        if (!empty($params["logoUrl"])) {
            $this->setImage($params["logoUrl"]);
        }
        ...
        if (!empty($params["containerBackgroundColor"])) {
            $this->setContainerBackgroundColor($params["containerBackgroundColor"]);
        }
        if (!empty($params["buttonTextColor"])) {
            $this->setDefaultButtonTextColor($params["buttonTextColor"]);
        }
        if (!empty($params["buttonBackgroundColor"])) {
            $this->setDefaultButtonBackgroundColor($params["buttonBackgroundColor"]);
        }
    }
