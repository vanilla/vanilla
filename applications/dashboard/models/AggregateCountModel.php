...
class AggregateCountModel implements SystemCallableInterface
{
    ...
    {
        $this->currentBatch = $nextBatch = $batch;
        foreach ($options as $key => $option) {
            try {
                ...
                try {
                    ...
                    for ($i = $batch; $i <= $batchSize; $i++) {
                        $this->currentBatch = $i;
                        foreach ($this->processCountIterator($option) as $discussionColumnsYielded) {
            ...
            $table = $options[$key]["table"];
            unset($this->model[$table], $options[$key], $table);
            $nextBatch = 1;
        }
        $options = [];
     ...
     * @return Generator<int, LongRunnerItemResultInterface, string, string|LongRunnerNextArgs>
     */
    public function processCountIterator(array $options): Generator
    {
        $offset = ($this->currentBatch - 1) * $this->limit;
     ...
     */
    public function counts($table, $column, $from = false, $to = false, $max, $where = [])
    {
        $this->model[$table] = $this->model[$table] ?? $this->dbaModel->createModel($table);

        if (!method_exists($this->model[$table], "Counts")) {
            throw new \Gdn_UserException("The $table model does not support count recalculation.");
        }
        if ($table == "Category") {
            return $this->model[$table]->counts($column, $where);
        }
        return $this->model[$table]->counts($column, $from, $to, $max, $where);
    }
