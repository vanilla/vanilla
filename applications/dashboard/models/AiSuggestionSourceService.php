...
class AiSuggestionSourceService implements LoggerAwareInterface, SystemCallableInterface
{
    ...
    public function checkIfUserHasEnabledAiSuggestions(int $userID = null): bool
    {
        ...
        $suggestAnswersDefault = true;
        if (
            ($anonymize && $anonymizeUser[UserMetaModel::ANONYMIZE_DATA_USER_META] == "-1") ||
            $anonymizeUser[UserMetaModel::ANONYMIZE_DATA_USER_META] == "1"
        ) {
            $suggestAnswersDefault = false;
        }
        $suggestAnswers = $this->userMetaModel->getUserMeta($userID, "SuggestAnswers", $suggestAnswersDefault)[
            "SuggestAnswers"
        ];
    ...
    public function processResponses(array $discussion, array $potentialAnswers): array
    {
        ...
        foreach ($potentialAnswers as $item) {
            try {
                ...
                if (!in_array($answer["answer"], [null, "null"], true)) {
                    $result[] = ["summary" => $answer["answer"]] + $item;
                }
            } catch (Exception $e) {
...
$useBrEnglishPrompt
Answer the question using only the information available in the source text given below.
In your answer, do not mention any subjects not contained in the source text.
If the source text is not related to the question, or you can't use the source text to answer the question,
respond with a value of `null`.
Source Text:
$sourceText
PROMPT
        );
    ...
    {
        return Schema::parse(["answer:s|n"]);
    }
