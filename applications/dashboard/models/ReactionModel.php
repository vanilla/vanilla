...
class ReactionModel extends Gdn_Model implements EventFromRowInterface, LoggerAwareInterface
{
     ...
     * @return array
     */
    public function expandCommentReactions(array $rows, $expand = []): array
    {
        if (ModelUtils::isExpandOption("reactions", $expand)) {
            $attributes = array_column($rows, "attributes", "commentID");
            $schema = $this->getReactionSummaryFragment();
            array_walk($rows, function (&$row) use ($attributes, $schema) {
                $withAttributes = ReactionModel::addAttributes($row, $attributes[$row["commentID"]]);
     ...
     * @return array
     */
    public function expandDiscussionReactions(array $rows, $expand = []): array
    {
        if (ModelUtils::isExpandOption("reactions", $expand) && !empty($rows)) {
            $attributes = array_column($rows, "attributes", "discussionID");
            $schema = $this->getReactionSummaryFragment();
            $discussionIDs = array_column($rows, "discussionID");
            $userReactions = $this->getUserDiscussionTags(Gdn::session()->UserID, $discussionIDs);
            $noneLabel = Gdn::getContainer()
                ->get(LocaleInterface::class)
                ->translate("None");
            array_walk($rows, function (&$row) use ($attributes, $schema, $userReactions, $noneLabel) {
                $withAttributes = self::addAttributes($row, $attributes[$row["discussionID"]] ?? null);
                $summary = $this->getRecordSummary($withAttributes);
    ...
    public static function structure(Gdn_DatabaseStructure $structure)
    {
        ...
        if (!Gdn::config()->get("Reactions.MigrationCompleted")) {
            $isLegacyAddonEnabled = Gdn::config("EnabledPlugins.Reactions");
            ...
            Gdn::config()->set("Reactions.ShowBestOf", !$isInstalled || $isLegacyAddonEnabled);
            Gdn::config()->set("Reactions.MigrationCompleted", true);
        }
