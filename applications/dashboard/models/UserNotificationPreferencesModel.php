...
class UserNotificationPreferencesModel
{
    ...
    public function save(int $userID, array $userPrefs): array
    {
        ...
        foreach ($userPrefs as $key => $val) {
            ...
            $activityTypeID = $parts[1];

            if (!in_array($activityTypeID, $allPreferences)) {
                throw new NotFoundException("{$activityTypeID} is not a valid activity type");
            }
    ...
    public function getDefaults(): array
    {
        ...
        foreach ($this->activityService->getActivityTypeIDs() as $activityType) {
            $isDisabled = Gdn::config()->get("Preferences.Disabled.{$activityType}", 0);
            $defaultPreferences["Disabled.{$activityType}"] = $isDisabled;
            $defaultPreferences["Email.{$activityType}"] = $isDisabled
                ? 0
                : Gdn::config()->get("Preferences.Email.{$activityType}", 0);
            $defaultPreferences["Popup.{$activityType}"] = $isDisabled
                ? 0
                : Gdn::config()->get("Preferences.Popup.{$activityType}", 0);
        }
    ...
    public function saveDefaults(array $updatedDefaults)
    {
        ...
        foreach ($updatedDefaults as $key => $value) {
            ...
            if (!in_array($keyParts[1], $this->activityService->getActivityTypeIDs())) {
                throw new NotFoundException("'{$keyParts[1]}' is not a valid activity type");
            }
