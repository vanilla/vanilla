...
class ActivityModel extends Gdn_Model implements SystemCallableInterface
{
    ...
    public function decodeNotificationToken(string $token): array
    {
        ...
        if ($activityID == 0) {
            $notificationRow = ["notifyUserID" => $userID, "activityType" => $activityType, "ActivityTypeList" => []];
     ...
     *
     * @param array|object $activity Activity object.
     ...
     * @return string
     */
    public function getUnfollowCategoryLink($user, int $categoryID = 0, string $format = "html"): string
    {
        $categoryModel = Gdn::getContainer()->get(CategoryModel::class);
        $category = $categoryID > 0 ? $categoryModel->getID($categoryID, DATASET_TYPE_ARRAY) : ["name" => ""];
        $linkText = $categoryID > 0 ? t("Unfollow") : t("Unsubscribe");
        $activityTypes = $categoryID > 0 ? "FollowedCategory" : "DigestEnabled";
        $tokenData = [
            "UserID" => val("UserID", $user, false),
            "Name" => val("Name", $user, false),
            "PhotoUrl" => val("PhotoUrl", $user, false),
