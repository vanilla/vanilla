...
class ActivityModel extends Gdn_Model implements SystemCallableInterface
{
    ...
    /**
     * Get the notification preference for an activity type for a set of preferences or a user. Return the
     * default value if the preference isn't found.
     *
     * @param string $activityType
    ...
    public static function notificationPreference($activityType, $preferencesOrUserID, $type = null)
    {
        ...
        if ($type === null) {
            ...
            return $result;
        } elseif ($type === "Popup" || $type === "Email") {
            if (isset($preferences[$type . "." . $activityType])) {
                return (bool) $preferences[$type . "." . $activityType];
            }
        }
    ...
    public function getNotificationData(stdClass $payload): array
    {
        ...
        if ($activityID == 0) {
            $notificationRow = ["notifyUserID" => $userID, "activityType" => "Digest", "ActivityTypeList" => []];
            if ($activityType == "FollowedCategory" && $categoryID > 0) {
                $notificationRow["data"] = val("ActivityData", $payload);
                $notificationRow["reason"] = "advanced";
            }
            if ($activityType == "DigestEnabled") {
                $notificationRow["data"] = val("ActivityData", $payload);
                $notificationRow["reason"] = $activityType;
            }
        } else {
     ...
     *
     * @param int $activityID An activityID.
     * @param array{UserID: int, Name: string, Email: string, PhotoUrl: string} $user User object.
     * @param string $format Email format.
     * @return string
     */
    public function getUnsubscribeLink(int $activityID, array $user, string $format): string
    {
        $activity = $this->getID($activityID, DATASET_TYPE_ARRAY);
        ...
        if ($activityType == "Comment") {
            ...
            if (in_array("bookmark", $activityData["reasons"])) {
                $tokenData["ActivityTypes"][] = "BookmarkComment";
                $tokenData["ActivityType"] = "BookmarkComment";
            }
        }
    ...
    {
        $token = JWT::encode($tokenData, $this->unsubscribeSalt, Gdn_CookieIdentity::JWT_ALGORITHM);
        return url("/unsubscribe/$token", "https");
    }
     ...
     *
     * @param array{UserID: int, Name: string, Email: string, PhotoUrl: string} $user User object.
     * @param int $categoryID Categorty ID.
     *
     * @return string
     */
    public function getUnfollowCategoryLink(array $user, int $categoryID): string
    {
        $categoryModel = Gdn::getContainer()->get(CategoryModel::class);
        if ($categoryID == 0) {
            throw new InvalidArgumentException('Invalid Category ID must be provided".', 400);
        }
    ...
    /**
     * Generate unsubscribe digest link.
     *
     * @param array{UserID: int, Name: string, Email: string, PhotoUrl: string} $user User object.
     *
     * @return string
    ...
    public function getUnsubscribeDigestLink(array $user): string
    {
        ...
        $tokenData = [
            ...
            "ActivityType" => "DigestEnabled",
            "ActivityID" => 0,
        ];

        $token = JWT::encode($tokenData, $this->unsubscribeSalt, Gdn_CookieIdentity::JWT_ALGORITHM);

        $linkUrl = url("/unsubscribe/$token", "https");
        return $linkUrl;
    }
    ...
    public function comment($comment)
    {
        ...
        if ($this->validate($comment)) {
            ...
            if ($iD) {
                ...
                if (in_array(val("ActivityType", $activity), ["WallPost", "Status"])) {
    ...
    public function queueNotification($activityID, $story = "", $position = "last", $force = false)
    {
        ...
        if ($user) {
            ...
            if ($preference) {
                ...
                $email->to($user);
                $url = externalUrl(val("Route", $activity) == "" ? "/" : val("Route", $activity));
                $footerText = $email->getFooterContent();
                $footerText = $footerText ? $footerText . " <br/>" : "";
    ...
    public function queue($data, $preference = false, $options = [])
    {
        ...
        if ((self::$Queue[$data["NotifyUserID"]][$data["ActivityType"]] ?? null) == null) {
            self::$Queue[$data["NotifyUserID"]][$data["ActivityType"]] = [$data, $options];
        } else {
            self::$Queue[$data["NotifyUserID"]][$data["ActivityType"]][0]["Data"]["Reason"] .=
                "," . $data["Data"]["Reason"];

            self::$Queue[$data["NotifyUserID"]][$data["ActivityType"]][0]["Data"]["Preference"] .=
    ...
    public function save($formPostValues, $settings = false, $options = [])
    {
        ...
        $activity["Data"] = !isset($activity["Data"]) ? ["Reason" => $activity["ActivityType"]] : $activity["Data"];
        $activity["Data"]["Preference"] = !isset($activity["Data"]["Preference"])
            ? $activity["ActivityType"]
            : $activity["Data"]["Preference"] . ", {$activity["ActivityType"]}";
        $activity["Data"]["Reason"] = !isset($activity["Data"]["Reason"])
            ? $activity["ActivityType"]
    ...
    private function handleDuplicateActivityForEvent(array $activity): ?array
    {
        ...
        foreach ($existingRows as $existingActivity) {
            ...
            $existingPreference = (array) ($existingData["Preference"] ?? null);
            $updatedReasons = $existingReasons;
            $updatedPreferences = $existingPreference;
            $existingHadInApp = $existingActivity["Notified"] !== self::SENT_SKIPPED;
            $existingHadEmail = $existingActivity["Emailed"] !== self::SENT_SKIPPED;

            if ($existingHadInApp || $existingHadEmail) {
                $activity["Notified"] = $existingHadInApp ? self::SENT_SKIPPED : $activity["Notified"];
                $activity["Emailed"] = $existingHadEmail ? self::SENT_SKIPPED : $activity["Emailed"];
                if ($activity["Data"]["Reason"] ?? false) {
                    $updatedReasons[] = $activity["Data"]["Reason"];
                }
                if ($activity["Data"]["Preference"] ?? false) {
                    $updatedPreferences[] = $activity["Data"]["Preference"];
                }
            }

            if (count($updatedReasons) > 1 || count($updatedPreferences) > 1) {
                ...
                $newData = array_merge($existingData, [
                    ...
                    "Preference" => implode(", ", array_unique(array_filter($updatedPreferences))),
                ]);
    ...
    public function normalizeNotificationRow(array $row): array
    {
        ...
        $row["preference"] = $row["Data"]["Preference"] ?? null;
