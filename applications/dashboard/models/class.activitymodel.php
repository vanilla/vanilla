...
class ActivityModel extends Gdn_Model implements SystemCallableInterface
{
    ...
    public function queue($data, $preference = false, $options = [])
    {
        ...
        if (!empty($preference)) {
            $data = $this->calculateActivityPreference($data, $preference, $options);
        }
    ...
    /**
     * Set user preference based properties of a new activity record.
     *
     * @param array $activity
     ...
     */
    private function calculateActivityPreference(array $activity, string $preference, array $options): array
    {
        if ($options["Force"] ?? false) {
            // The "Force" flag is to bypass skipping notifying a user about their own activity. If they have a preference
            // set, we still want to respect it.
            $optOuts = $this->checkOptOuts($activity);
            $activity["Notified"] = $optOuts["Popup"] ? self::SENT_SKIPPED : self::SENT_PENDING;
            $activity["Emailed"] = $optOuts["Email"] ? self::SENT_SKIPPED : self::SENT_PENDING;
            return $activity;
        }
    ...
    /**
     * Check to see if a user has specifically opted out of receiving a notification.
     *
     * @param array $activity
     * @return bool[]
     */
    private function checkOptOuts(array $activity): array
    {
        $activityType = $activity["ActivityType"] ?? ($activity["activityType"] ?? false);
        if (!$activityType) {
            $optOuts["Popup"] = false;
            $optOuts["Email"] = false;
            return $optOuts;
        }
        $optOuts = ["Popup" => false, "Email" => false];
        ...
        $defaultPreferences = [
            "Popup.{$activity["ActivityType"]}" => Gdn::config("Preferences.Popup.{$activity["ActivityType"]}"),
            "Email.{$activity["ActivityType"]}" => Gdn::config("Preferences.Email.{$activity["ActivityType"]}"),
        ];
        $userPreferences = $user["Preferences"];
        if (!isset($userPreferences)) {
            $userPreferences = $defaultPreferences;
        }
        $optOuts["Popup"] = !($userPreferences["Popup.{$activity["ActivityType"]}"] ?? false);
        $optOuts["Email"] = !($userPreferences["Email.{$activity["ActivityType"]}"] ?? false);
    ...
    public function save($formPostValues, $settings = false, $options = [])
    {
        ...
        if (!empty($preference)) {
            $activity = $this->calculateActivityPreference($activity, $preference, $options);
        }
    ...
    private function queueEmail(array $activity, array $options = [])
    {
        ...
        $notifyUserID = $activity["NotifyUserID"] ?? null;
        if (
            filter_var($activityID, FILTER_VALIDATE_INT) === false ||
            filter_var($notifyUserID, FILTER_VALIDATE_INT) === false
        ) {
            return;
        }
        ...
        $activityEmail->addActivityIDWithRecipient($activityID, $address, $user["Name"] ?? null);
    }
    ...
    private function sendActivityEmail(ActivityEmail $activityEmail)
    {
        ...
        while ($batch = array_slice($recipients, $batchOffset, $batchSize)) {
            ...
            foreach ($batch as $recipient) {
                [$activityIDs[], $address, $name] = $recipient;
            ...
            $this->SQL->put("Activity", ["Emailed" => $status], ["ActivityID" => $activityIDs]);
