...
class ActivityModel extends Gdn_Model
{
    ...
    public function getWhere(
        ...
        if ($shouldBatch && isset($where["a.NotifyUserID"])) {
            $innerSelectSql = <<<SQL
select min(a3.ActivityID) as `ActivityID`, max(a3.DateUpdated) as `MaxDateUpdated`, count(*) as `count`
from `GDN_Activity` `a3`
where `a3`.`NotifyUserID` = {$where["a.NotifyUserID"]}
group by coalesce(`a3`.`ParentRecordID`, `a3`.`RecordID`, - `a3`.`ActivityID`), `a3`.`ActivityTypeID`, `a3`.`RecordType`,`a3`.`Notified`, `a3`.`BatchID`
SQL;

            $this->SQL->select("aj.*")->join("($innerSelectSql) aj", "a2.ActivityID = aj.ActivityID");
        }
     ...
     * @return array
     */
    public function getNotificationWithCount(int $id): array
    {
        ...
        $notification = $this->getWhere($where, "", "", false, false, true)->firstRow();
    ...
    public function markAllRead($userID)
    {
        ...
        $activities = $this->getWhere([
            "NotifyUserID" => $userID,
            "Notified" => [self::SENT_PENDING, self::SENT_TOAST, self::SENT_NONE],
        ]);
        $singleActivities = [];
        ...
        if (!empty($singleActivities)) {
            $this->SQL->put("Activity", ["Notified" => self::SENT_OK], ["ActivityID" => $singleActivities]);
        }
