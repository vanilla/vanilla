...
SQL;
    ...
    public function sendNotification($ActivityID, $Story = "", $Force = false)
    {
        ...
        if ($User) {
            if ($Force) {
                ...
                $Preference = val(
                    "Email." . $Activity->ActivityType,
                    $Preferences,
                    Gdn::config("Preferences.Email." . $Activity->ActivityType)
                );
            }
            if ($Preference) {
    ...
    public function queue($data, $preference = false, $options = [])
    {
        ...
        if (isset(self::$Queue[$data["NotifyUserID"]][$data["ActivityType"]])) {
            ...
            $notified = $notified ?: $currentData["Notified"];
            $emailed = $emailed ?: $currentData["Emailed"];
        ...
        if (!$activityID) {
            if (!$delete) {
                ...
                if ($activity["Notified"] === self::SENT_PENDING || $activity["Emailed"] == self::SENT_PENDING) {
                    $eventActivity = $this->getWhere(["ActivityID" => $activityID], "", "", 1)->firstRow() ?? [];
