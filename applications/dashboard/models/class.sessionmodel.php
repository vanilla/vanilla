...
class SessionModel extends Gdn_Model
{
     ...
     * @return int
     */
    public function getPersistExpiry(): int
    {
        return strtotime(\Gdn::config("Garden.Cookie.PersistExpiry", "30 days"));
    }
    ...
    public function startNewSession(int $userID, string $sessionID = null)
    {
        ...
        if (!$session) {
            $session = [
                ...
                "DateExpires" => date(MYSQL_DATE_FORMAT, $this->getPersistExpiry()),
                "Attributes" => [],
            ];
            if ($sessionID != null) {
    ...
    public function refreshSession(string $sessionID): bool
    {
        ...
        $halfTime = ($this->getPersistExpiry() - time()) / 2;
        if (strtotime($session["DateExpires"]) < time() + $halfTime) {
            $fields = [
                "DateExpires" => date(MYSQL_DATE_FORMAT, $this->getPersistExpiry()),
