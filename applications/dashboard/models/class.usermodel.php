    ...
    FragmentFetcherInterface
{
    ...
    private function insertInternal($fields, $options = [])
    {
        ...
        if ($userID) {
            ...
            $user = $this->getID($userID);
            $userEvent = $this->eventFromRow((array) $user, UserEvent::ACTION_INSERT, $this->currentFragment());
    ...
    /**
     * Join in user profile fields for a list of users.
     *
     * @param array $users
     * @return void
     */
    public function joinProfileFields(array &$users)
    {
        $userIDs = array_column($users, "UserID");
        $usersProfileFields = $this->profileFieldModel->getUsersProfileFields($userIDs);
        foreach ($users as &$user) {
            if (isset($usersProfileFields[$user["UserID"]])) {
                $user["profileFields"] = $usersProfileFields[$user["UserID"]];
            }
        }
    }
    ...
    public static function givePoints(int $userID, int $points, $source = "Other", $timestamp = false)
    {
        ...
        $totalPoints = Gdn::sql()
            ->getWhere("UserPoints", ["UserID" => $userID, "SlotType" => "a", "Source" => "Total", "CategoryID" => 0])
            ->value("Points");
    ...
    public function readSchema()
    {
        ...
        if ($this->session->checkPermission("site.manage")) {
            $adminOnlySchema = Schema::parse(["lastIPAddress?"]);
    ...
    public function search($filter, $orderFields = "", $orderDirection = "asc", $limit = false, $offset = false)
    {
        ...
        if (is_array($filter)) {
            ...
            $this->profileFieldModel->applyExtendedFilter($this->SQL, $where);
        } else {
            $keywords = $filter;
        }
        ...
        if (!empty($roleID)) {
            ...
            if ($optimize && !$isIPAddress) {
                ...
                $whereCriterias = $this->getEventManager()->fireFilter(
    ...
    public function filterPrivateUserRecord(array &$rowOrRows)
    {
        ...
        $filterRow = function (&$row) use ($isPrivateBansEnabled) {
            ...
            if (($isUserPrivate || $isPrivateBanned) && !$isOwnProfile) {
                $row = ArrayUtils::pluck($row, ["userID", "name", "banned", "photoUrl", "private"]);
            }
        };
    ...
    public function triggerApplicantNotification($userID = 0, $username = "")
    {
        if ($userID > 0 && !empty($username)) {
            ...
            try {
                ...
                Logger::event("applicant_notification_failure", Logger::ERROR, $ex->getMessage());
            }
        }
    }
            ...
            try {
                ...
                if ($log) {
                    if ($ex->getCode() === Gdn_Email::ERR_SKIPPED) {
                        Logger::event("password_reset_skipped", Logger::INFO, $ex->getMessage(), [
                            "input" => $input,
                            "email" => $user->Email,
                            Logger::FIELD_CHANNEL => Logger::CHANNEL_SECURITY,
                        ]);
