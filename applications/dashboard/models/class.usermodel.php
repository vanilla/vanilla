    ...
    private function joinUserMeta(&$user): void
    {
        ...
        if (is_array($user) && ($user["UserID"] ?? false)) {
            ...
            if (
                ...
                Gdn::config("Feature.ImprovedUserProfileFields.Enabled")
            ) {
                // We go through the fields that should be defined as user meta.
                foreach (self::USERMETA_FIELDS as $userMetaField) {
                    // If that user has a user meta value coming from the `User` table
                    if ($user[$userMetaField] ?? false) {
            ...
            if (count($userMeta) > 0) {
                $user = array_merge($user, array_intersect_key($userMeta, $user));
            }
        }
    }
}
