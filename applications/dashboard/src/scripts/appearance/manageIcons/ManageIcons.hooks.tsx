...
import { promiseTimeout, uuidv4 } from "@vanilla/utils";
...
export function useParseBulkIconZip() {
    ...
    return useMutation({
        mutationFn: async (file: File): Promise<IParsedBulkIcons> => {
            ...
            const countSvgs = Object.values(zip.files).filter((file) => file.name.endsWith(".svg")).length;
            if (countSvgs === 0) {
