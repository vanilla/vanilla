...
export function AutomationRulesAddEditImpl(props: { automationRuleID?: string }) {
    ...
    const handleErrors = (err) => {
        // some adjustments for top level errors, if its not a per field error, find and show more specific error on top
        const hasErrorsPerField = err.errors && Object.keys(err.errors).find((key) => key && key !== "");
        const topLevelErrorMessage =
            !hasErrorsPerField && err.errors && err.errors[""] && err.errors[""][0]
