...
function useSettingsNav(): INavigationTreeItem[] | null {
    const sections = useDashboardSection();

    const moderationNav = sections.data?.find((section) => section.id === "settings");
    if (!moderationNav) {
        return null;
    }
    ...
    if (props.asHamburger) {
        return (
            <>
                ...
                <Heading title={t("Settings")} className={dropdownClasses.sectionHeading} />
