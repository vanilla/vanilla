...
export function EmailSettings() {
    ...
    useEffect(() => {
        if (value["emailDigest.dayOfWeek"]) {
            const weekDay = parseInt(value["emailDigest.dayOfWeek"].toString(), 10);
    ...
    let sections = [t(EMAIL_STYLES_SECTION), t(OUTGOING_EMAILS_SECTION), t(EMAIL_NOTIFICATIONS_SECTION)];

    if (isDigestEnabled && value["emailNotifications.disabled"]) {
        sections.push(t(EMAIL_DIGEST_SECTION));
    }
