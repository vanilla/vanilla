...
export function DashboardFormControlGroup(props: React.PropsWithChildren<IControlGroupProps> & IFormGroupProps) {
    ...
    const { label, description, fullSize, inputType, tooltip } = controls[0];
    if (fullSize || inputType === "upload") {
        return <>{children}</>;
    }
    return (
        <DashboardFormGroup label={label ?? ""} description={description} inputType={inputType} tooltip={tooltip}>
            {children}
        </DashboardFormGroup>
    );
}
