    ...
    useEffect(() => {
        if (layoutID) {
            if (layoutDraftLoadable?.status) {
                if (
                    layoutDraftLoadable.status === LoadStatus.SUCCESS &&
                    layoutDraftLoadable.data.layoutID !== layoutID
                ) {
                    // use existing layout, if it's available
                    dispatch(layoutActions.initializeLayoutDraft({ initialLayout: layoutDraftLoadable.data }));
                }
            } else {
