...
export function LayoutEditorWidgetToolbar(props: IProps) {
    ...
    const { $hydrate, $middleware, ...widgetProps } = widgetSpec ?? {};
    ...
    return (
        <>
            <EmbedMenu
                ...
                className={layoutEditorClasses().toolbarMenu}
            >
                <EmbedButton
                    disabled={!pathLeft}
                    onClick={() => {
            ...
            <WidgetSettingsModal
                ...
                widgetSchema={catalog?.widgets[$hydrate as string]["schema"] ?? {}}
            />
        </>
    );
}
