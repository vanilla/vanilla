...
function ApplyLayoutImpl(props: { layout: ILayoutDetails }) {
    const { layout } = props;
    const putLayoutViews = usePutLayoutViews(layout.layoutID);
    const toast = useToast();
    const { patchConfig } = useConfigPatcher();

    const viewIsAlreadyApplied = (layout?.layoutViews || []).some(
        (layoutView) =>
            layoutView.recordType === GLOBAL_LAYOUT_VIEW.recordType &&
            layoutView.recordID === GLOBAL_LAYOUT_VIEW.recordID,
    );
