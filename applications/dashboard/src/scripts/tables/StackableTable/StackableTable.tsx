                    ...
                    {notWrappedColumns.map((columnName, key) => {
                        ...
                        return (
                            <td key={key} role="cell">
                                ...
                                {!isFirstColumn && <LoadingRectangle height={15} width={90} />}
                            </td>
                        );
                    })}
                    {ActionsCellRenderer && (
...
export default function StackableTable(props: IStackableTableProps) {
    const { onHeaderClick, columnsConfiguration, hiddenHeaders, className, ...rest } = props;
...
function adjustColumnsVisibility(
    ...
    if (actualWidth > 0) {
        ...
        const notWrappedColumnsNumber =
            spaceForVisibleColumns > 0
                ? Math.floor(spaceForVisibleColumns / ((DEFAULT_COLUMN_MAX_WIDTH + DEFAULT_COLUMN_WIDTH) / 2))
                : 0;
        const newConfiguration = { ...configuration };
