...
const renderInProvider = (props: Partial<React.ComponentProps<typeof DashboardAddEditUser>> = {}) => {
    render(
        EditProfileFieldsFixture.createMockProfileFieldsProvider(<DashboardAddEditUser {...props} />, {
            users: {
                usersByID: {
                    2: {
...
describe("AddEditUserModal", () => {
    it("Adding user, empty fields", async () => {
        ...
        //we successfully opened the modal, close button is there
        const modalCloseButton = await screen.findByLabelText("Close");
    ...
    it("On user add/edit pages/url, we should not render the modal but the form only, ", async () => {
        renderInProvider({
            userData: { userID: 2, name: "test-user-no-modal", email: "test-user-no-modal@example.com" },
            profileFields: ProfileFieldsFixture.mockProfileFields() as ProfileField[],
            isAddEditUserPage: true,
        });

        //no modal
        const modalCloseButton = await screen.queryByLabelText("Close");
        expect(modalCloseButton).not.toBeInTheDocument();

        //fields still need to be present and pre-populated
        const userNameInput = await screen.findByDisplayValue("test-user-no-modal");
        const emailInput = await screen.findByDisplayValue("test-user-no-modal@example.com");
        const profileField = await screen.getByDisplayValue(
            EditProfileFieldsFixture.mockFields.profileFieldsByUserID[2].data!.text,
        );

        expect(userNameInput).toBeTruthy();
        expect(emailInput).toBeTruthy();
        expect(profileField).toBeTruthy();
    });
});
