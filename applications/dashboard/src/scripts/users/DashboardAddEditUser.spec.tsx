...
import { RenderResult, act, cleanup, fireEvent, render, screen, within } from "@testing-library/react";
...
describe("Editing a user", () => {
    ...
    it.only("The user data fields are pre-filled.", async () => {
        // expect.assertions(mockProfileFields.length + 2);
        ...
        mockProfileFields.forEach(({ apiName, label, formType }) => {
            if ([ProfileFieldFormType.DROPDOWN, ProfileFieldFormType.TOKENS].includes(formType)) {
                // FIXME: We can't locate an HTML element that would reflect the current value of the Combobox
                return;
            }
