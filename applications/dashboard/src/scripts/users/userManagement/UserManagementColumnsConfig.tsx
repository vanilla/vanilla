...
export default function UserManagementColumnsConfig(props: IProps) {
    ...
    const handleSubmit = () => {
        ...
        treeItemIDs.forEach((itemID, index) => {
            ...
            const columnProfileField = profileFields?.filter((profileField) => profileField.label === itemData.column);
            newConfiguration[itemData.column] = {
                ...configuration[itemData.column],
                order: index + 1,
                isHidden: itemData.isHidden ?? false,
                wrapped: false,
                columnID: columnProfileField && columnProfileField.length ? columnProfileField[0].apiName : undefined,
