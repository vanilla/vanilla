...
export default function DashboardAddEditUser(props: IDashboardAddEditProps) {
    const { userData, ranks, isAddEditUserPage, newUserManagement, profileFields, minPasswordLength } = props;
    ...
    if (isEdit && userData?.userID) {
        ...
        const commonEditFormProps = {
            ...
            userID,
            initialValues: {
                ...mapUserDataToFormValues(userData as IUserDataProps, ranks),
                ...(!!profileFields &&
                    !!userData.profileFields && {
                        profileFields: transformUserProfileFieldsData(userData.profileFields, profileFields),
                    }),
            },
            title: t("Edit User"),
        ...
        return (
            <>
                ...
                {(modalVisible || isAddEditUserPage) && (
                    <>
                        {userData?.profileFields ? (
                            <DashboardEditUser_New {...commonEditFormProps} />
                        ) : (
                            <UserProfileFieldsContextProvider userID={userID}>
                                <DashboardEditUser_Legacy {...commonEditFormProps} profileFields={profileFields} />
                            </UserProfileFieldsContextProvider>
                        )}
                    </>
                )}
...
function DashboardEditUser_Legacy(
    ...
    let { userID, initialValues, profileFields, ...otherProps } = props;
    ...
    initialValues = {
        ...initialValues,
        profileFields: {
            ...(!!userProfileFields.data && !!profileFields
                ? transformUserProfileFieldsData(userProfileFields.data, profileFields)
                : {}),
        },
    };
