...
/**
 * Mock client used to test the RagSearchClient.
 */
class NexusRagSearchMockClient extends NexusRagSearchClient
{
    ...
    public function __construct(
        ...
        $config->saveToConfig([
            NexusRagSearchClient::NEXUS_RAG_URL_CONFIG_KEY => "http://nexus-rag-mock-url",
            NexusRagSearchClient::NEXUS_AUTH_URL_CONFIG_KEY => "http://nexus-rag-mock-auth-url",
            NexusRagSearchClient::NEXUS_RAG_CLIENT_ID_CONFIG_KEY => "test-client-id",
            NexusRagSearchClient::NEXUS_RAG_CLIENT_SECRET_CONFIG_KEY => "test-client-secret",
            NexusRagSearchClient::NEXUS_RAG_MODEL_CONFIG_KEY => "test-recipe-code",
        ]);

        $httpClient = Gdn::getContainer()->get(MockHttpClient::class);
        parent::__construct($httpClient, $this->config, $siteProvider, $this->aiConversationModel);
    ...
    {
        $token = MD5(microtime());
        $this->config->saveToConfig(self::NEXUS_RAG_TOKEN_CONFIG_KEY, $token);
        $this->config->saveToConfig(self::NEXUS_RAG_TOKEN_EXPIRATION_CONFIG_KEY, time() + 3600);
        return $token;
    }
    ...
    public function setMockResponses(): void
    {
        ...
        $messages = array_merge($messages, [
            ...
            $this->createMockMessage(["role" => "assistant", "message" => "42"]),
        ]);
        $this->createMockResponse(
            "POST",
