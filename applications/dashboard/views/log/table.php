    ...
    <div class="table-wrap">
        <table id="Log" class="table-data table-data-content js-tj">
            ...
            <tr>
                ...
                <th class="UsernameCell column-lg username-cell-responsive"><?php echo $this->data(
                    "_flaggedByTitle",
                    t("Flagged By", "Flagged By")
                ); ?></th>
                <th class="options column-checkbox"></th>
            </tr>
            </thead>
            <tbody>
            <?php foreach ($this->data("Log") as $row):
                ...
                <tr id="<?php echo "LogID_{$row["LogID"]}"; ?>">
                    <td class="column-checkbox">
                        <?php if (!in_array($row["RecordType"], ["Configuration", "Spoof"])) { ?>
                        <input type="checkbox" name="LogID[]" value="<?php echo $row["LogID"]; ?>"/>
                        <?php } ?>
                    </td>
                    <td class="content-cell">
                        <?php
                        $recordUser = Gdn::userModel()->getID(
                            $row["Data"]["InsertUserID"] ?? $row["RecordUserID"],
                            DATASET_TYPE_ARRAY
                        );
                        if ($row["RecordName"]) {
                            ...
                            $authorBlock
                                ...
                                ->addTitleMetaIf(
                                    (bool) $recordUser["Banned"],
                                    wrap(t("Banned"), "span", ["class" => "text-danger"])
                                )
                                ->addTitleMeta(
                                    plural(
                                        $recordUser["CountDiscussions"] + $recordUser["CountComments"],
                                        "%s post",
                                        "%s posts"
                                    )
                                )
                                ...
                                ->addMeta(
                                    Gdn_Format::dateFull($row["Data"]["DateInserted"] ?? $row["RecordDate"], "html")
                                )
                                ->addMeta("<b>" . htmlspecialchars(t($recordLabel, $recordLabel)) . "</b>")
                                ->addMetaIf(
                                    $viewPersonalInfo && !empty($row["Data"]["InsertIPAddress"]),
                                    iPAnchor($row["Data"]["InsertIPAddress"])
                                );
