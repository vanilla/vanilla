...
$desc = t(
    'Roles determine user\'s permissions.',
    "Every user in your site is assigned to at least one role. Roles are used to determine what the users are allowed to do."
);

$links = "<ul>";
$links .= wrap(
    anchor(t("Video tutorial on managing roles &amp; permissions"), "settings/tutorials/roles-and-permissions"),
    "li"
);
$links .= wrap(
    anchor("Default Role Types", "https://success.vanillaforums.com/kb/articles/39-roles-permissions#default-types"),
    "li"
);
$links .= "</ul>";
...
?>
<?php
echo heading(t("Manage Roles & Permissions"), [
    [
            ...
            <tr id="<?php echo $Role["RoleID"]; ?>"<?php echo $Alt ? ' class="Alt"' : ""; ?>>
                ...
                <td class="options">
                    ...
                    <?php if ($Role["CanModify"]) {
                        echo anchor(dashboardSymbol("edit"), "/role/edit/{$Role["RoleID"]}", "btn btn-icon", [
                            "aria-label" => t("Edit"),
                            "title" => t("Edit"),
                        ]);
                        if ($Role["Deletable"]) {
