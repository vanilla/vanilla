    ...
    <li id="<?php echo "Provider_$Index"; ?>" class="media <?php echo $CssClasses; ?>">
        ...
        <div class="media-right media-options">
            ...
            if ($Enabled) {
                ...
                $toggleState = 'on';
                echo wrap(anchor('<div class="toggle-well"></div><div class="toggle-slider"></div>', "/social/disable/$Index", 'Hijack', ['aria-label' =>sprintf(t('Disable %s'), val('Name', $Connection))]), 'span', array('class' => "toggle-wrap toggle-wrap-{$toggleState}"));
            } else {
                $toggleState = 'off';
                echo wrap(anchor('<div class="toggle-well"></div><div class="toggle-slider"></div>', "/social/enable/$Index", 'Hijack', ['aria-label' =>sprintf(t('Enable %s'), val('Name', $Connection))]), 'span', array('class' => "toggle-wrap toggle-wrap-{$toggleState}"));
            }
            ?>
            </div>
        </div>
