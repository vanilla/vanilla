...
class DigestEmail extends \Gdn_Email implements LoggerAwareInterface
{
     ...
     */
    public function __construct(\ActivityModel $activityModel, ConfigurationInterface $config)
    {
    ...
    {
        if ($this->format === "html") {
            $this->PhpMailer->msgHTML($this->getHtmlContent());
            $this->PhpMailer->AltBody = trim($this->getTextContent());
        } else {
            $this->PhpMailer->Body = trim($this->getTextContent());
        }
    }
