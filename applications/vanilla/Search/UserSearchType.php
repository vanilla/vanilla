...
class UserSearchType extends AbstractSearchType
{
    ...
    public function getQuerySchema(): Schema
    {
        $schemaFields = array_merge(
            [
                ...
                "profileFields:o?" => $this->buildProfileFieldSchema(),
            ],
            $this->schemaFields
        );
     ...
     * @return Schema
     */
    private function buildProfileFieldSchema(): Schema
    {
        $schemaArray = [];

        foreach ($this->profileFieldModel->getProfileFields() as $field) {
            $name = $field["apiName"];
     ...
     * @throws ForbiddenException|ClientException
     */
    ...
    private function validateProfileFieldFilters(SearchQuery $query)
    {
        ...
        $databaseProfileFields = $this->profileFieldModel->getProfileFields(["enabled" => true]);
        $databaseProfileFieldsIndexed = array_column($databaseProfileFields, null, "apiName");

        foreach (array_keys($profileFields) as $apiName) {
            if (!isset($databaseProfileFieldsIndexed[$apiName])) {
                throw new NotFoundException("No profile field found with name $apiName");
            }
            if (!$this->profileFieldModel->canView(null, $databaseProfileFieldsIndexed[$apiName])) {
                throw new ForbiddenException("You do not have permission to search by that profile field");
            }
        }
    }
    ...
    private function applyProfileFieldFilters(SearchQuery $query)
    {
        ...
        $databaseProfileFields = $this->profileFieldModel->getProfileFields();
        $databaseProfileFieldsIndexed = array_column($databaseProfileFields, null, "apiName");
        foreach ($profileFields as $apiName => $value) {
            $databaseProfileField = $databaseProfileFieldsIndexed[$apiName] ?? null;
