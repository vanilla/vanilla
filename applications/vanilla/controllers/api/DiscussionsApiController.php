...
class DiscussionsApiController extends AbstractApiController {
    ...
    public function delete($id): Data {
        ...
        $result = $this->discussionModel->deleteID($id);
        if ($result instanceof TrackingSlipInterface) {
            return new JobScheduledResponseData($result);
        } else {
            // Discussion was deleted immediately.
     ...
     * @throws ClientException If the destination discussion is not found.
     */
    public function patch_merge(array $body) {
        ...
        $discussion = $this->discussionModel->getID($destinationDiscussionID, DATASET_TYPE_ARRAY);
        if (!$discussion) {
            throw new ClientException('Destination discussion is not found.');
        }
        $isRedirect = $body['isRedirect'] ?? false;
