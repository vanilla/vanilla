...
class DraftsApiController extends \AbstractApiController
{
    ...
    public function patch_schedule(int $draftID, array $body): Data
    {
        ...
        if ($this->getPermissions()->has("silentPosting.allow")) {
            $in->merge(Schema::parse(["publishedSilently:b?"]));
        }
        ...
        if (isset($body["publishedSilently"])) {
            $currentValue = $draft["attributes"]["draftMeta"]["publishedSilently"] ?? false;
            if ($currentValue !== $body["publishedSilently"]) {
                $draft["attributes"]["draftMeta"]["publishedSilently"] = $body["publishedSilently"];
                $this->patch($draftID, $draft);
            }
        }
