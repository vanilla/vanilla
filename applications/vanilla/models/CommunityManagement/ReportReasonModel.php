...
class ReportReasonModel extends FullRecordCacheModel
{
    ...
    public function __construct(\Gdn_Cache $cache, private \Gdn_Session $session, private \UserModel $userModel)
    {
        ...
        $this->addPipelineProcessor(new Operation\BooleanFieldProcessor(["deleted"]));
     ...
     * @return array
     */
    public function getPermissionAvailableReasonIDs(): array
    {
        $reasons = $this->select(
            where: [
                "deleted" => false,
            ],
            options: [self::OPT_SELECT => ["reportReasonID", "roleIDs"]]
        );

        $result = [];
        $isCommunityManager = \Gdn::session()->checkPermission("community.manage");
        $userRoleIDs = $this->userModel->getRoleIDs($this->session->UserID) ?: [];
        foreach ($reasons as $reason) {
            $roleIDs = $reason["roleIDs"] ?? [];
            if ($isCommunityManager || empty($roleIDs) || count(array_intersect($roleIDs, $userRoleIDs)) > 0) {
