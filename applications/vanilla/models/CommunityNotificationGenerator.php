...
class CommunityNotificationGenerator implements SystemCallableInterface
{
    ...
    {
        return ["processNotifications", "categoryNotificationsIterator"];
    }
    ...
    public function processBookmarkNotifications(array $notificationData, array $discussion): ?LongRunnerAction
    {
        ...
        $userIDs = $this->discussionModel->getBookmarkUserIDs($discussionID);
        if (empty($userIDs)) {
            return null;
        }
        $groupData = [
    ...
    {
        $userIDs = $this->discussionModel->getParticipatedUserIDs($discussion["DiscussionID"]);
        if (empty($userIDs)) {
            return null;
        }
    ...
    public function categoryNotificationsIterator(
        ...
        foreach ($userToNotifyByID as $userID => $notificationPrefs) {
            try {
                ...
                return new LongRunnerNextArgs([$activity, $discussion, $recordType, $userID]);
            } catch (\Exception $e) {
    ...
    public function processNotifications(
        ...
        foreach ($notifyUserIDs as $notifyUserID) {
            try {
                ...
                if (!$this->discussionModel->canView($discussion, $notifyUserID)) {
                    continue;
                }
                ...
                $processedNotifyUserIDs[] = $notifyUserID;
                yield new LongRunnerSuccessID(
                ...
                $idsToProcess = array_values(array_diff($notifyUserIDs, $processedNotifyUserIDs));
                $groupData["notifyUserIDs"] = $idsToProcess;
                return new LongRunnerNextArgs([$notificationData, $reason, $groupData, $discussion]);
            } catch (\Exception $e) {
                yield new LongRunnerFailedID(
                    "{$notificationData["RecordType"]}_{$notification["RecordID"]}_User_{$notifyUserID}_NotificationType_{$reason}"
                );
