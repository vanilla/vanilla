    ...
    SystemCallableInterface
{
        ...
        switch ($column) {
            ...
            case "ParentLastDateInserted":
                ...
                $lastDateInsertedData = $this->SQL
                    ...
                    ->get()
                    ->resultArray();
                foreach ($lastDateInsertedData as $key => $val) {
                    unset($lastDateInsertedData[$key]);
                    $lastDateInsertedData[$val["ParentCategoryID"]] = $val["LastDateInserted"];
                }
    ...
    public function refreshAggregateRecentPost($categoryID, $updateAncestors = false)
    {
        ...
        $discussion = $this->SQL
            ->getWhere("Discussion", ["CategoryID" => $categoryIDs], "DateLastComment", "desc", 1)
            ->firstRow(DATASET_TYPE_ARRAY);
        $comment = null;
    ...
    public function recalculateTree(array $where = []): array
    {
        ...
        $addWhere = function (string $sql, string $condition, string $whereAdd = "where") use ($where, &$params) {
            ...
            if ($condition == "c.CategoryID" && in_array(-1, $where["CategoryID"])) {
