    ...
    AggregateCountableInterface
{
    ...
    public function setPreferences(int $userID, int $categoryID, array $preferences): void
    {
        ...
        if ($currentlyFollowed != $followingPref || (!$currentlyFollowed && !$followingPref)) {
            ...
            if (!$followingPref) {
                $prefsToSet = [
                    sprintf(self::PREFERENCE_DIGEST_EMAIL, $categoryID) => false,
...
SQL;
    ...
    public static function sortFlatCategories(array &$categories): void
    {
        ...
        uasort($categories, function ($a, $b) use ($categories) {
            if (
                ...
                $categories[$a["ParentCategoryID"]]["DisplayAs"] !== self::DISPLAY_FLAT
            ) {
    ...
    public function calculateCategoryLayoutViewType(int $categoryID): string
    {
        ...
        } elseif (in_array($category["DisplayAs"], [Categorymodel::DISPLAY_FLAT, Categorymodel::DISPLAY_NESTED])) {
            $layoutViewType = Categorymodel::LAYOUT_NESTED_CATEGORY_LIST;
        } elseif ($category["DisplayAs"] == Categorymodel::DISPLAY_HEADING) {
            throw new Exception(t("Heading categories cannot be viewed directly."), 400);
        } else {
            $layoutViewType = Categorymodel::LAYOUT_CATEGORY_LIST;
        }
