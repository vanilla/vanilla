    ...
    UserMentionsInterface
{
    ...
    public function save2($commentID, $insert, $checkExisting = true, $incUser = false)
    {
        ...
        if (
            $validController &&
            Gdn::controller()->deliveryType() === DELIVERY_TYPE_ALL &&
            $fields["InsertUserID"] != $session->UserID
        ) {
            return;
        }
        ...
        if ($insert) {
            ...
            $this->notifyNewComment($fields ? (array) $fields : null, $discussion ? (array) $discussion : null);
        }
    }
    ...
    public function recordAdvancedNotications($activityModel, $activity, $discussion)
    {
        ...
        $data = $this->SQL
            ->whereIn("Name", [
                "Preferences.Email.NewComment." . $category["CategoryID"],
                "Preferences.Popup.NewComment." . $category["CategoryID"],
            ])
            ->get("UserMeta")
            ->resultArray();
        ...
        if ($discussion) {
            if ($data && $data["CountComments"] !== 0) {
                ...
                if (is_numeric($discussionID)) {
                    $this->discussionModel->updateColumnHot([$discussionID]);
                }
