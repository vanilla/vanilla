...
import {
    ...
    NestedCommentProviderProps,
    useNestedCommentContext,
} from "@vanilla/addon-vanilla/comments/NestedCommentContext";
...
import { isNestedComment } from "@vanilla/addon-vanilla/comments/NestedCommentUtils";
import { IThreadItemComment } from "@vanilla/addon-vanilla/comments/NestedCommentTypes";
...
function TestWrapper(props: PropsWithChildren<NestedCommentProviderProps>) {
    const { children, ...rest } = props;
    return (
        <QueryClientProvider client={queryClient}>
            <QueryClientProvider client={queryClient}>
                <NestedCommentContextProvider {...rest}>{children}</NestedCommentContextProvider>
            </QueryClientProvider>
        </QueryClientProvider>
    );
}
...
describe("CommentThreadContext", () => {
    ...
    it("Set initial state", () => {
        ...
        const { result } = renderHook(() => useNestedCommentContext(), {
            wrapper: TestWrapper,
            initialProps: {
    ...
    it("Fills holes", async () => {
        ...
        if (isNestedComment(firstComment)) {
        ...
        // Fill the hole
        await act(async () => {
            await result.current.addToThread("/comments/thread", parentID);
        });

        // Thread is the correct updated size
        const updatedFirstComment = result.current.threadStructure[0];
        if (isNestedComment(updatedFirstComment)) {
    ...
    it("Collapses thread", async () => {
        ...
        if (isNestedComment(result.current.threadStructure[0])) {
            expect(result.current.threadStructure[0]?.children?.length).toBe(2);
        }

        // Assert nested thread children
        if (isNestedComment(result.current.threadStructure[1])) {
