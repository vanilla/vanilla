...
import { useFetchGroupByID } from "@groups/groups/GroupList.hooks";
...
export function CreatePostFormAsset(props: IProps) {
    ...
    const initialGroupID =
        parentRecordContext?.parentRecordType === "group" ? parentRecordContext.parentRecordID : undefined;
    ...
    const groupID = useMemo(() => {
        return formBody.groupID ?? draft?.attributes?.groupID ?? initialGroupID ?? undefined;
    }, [formBody.groupID, draft?.attributes?.groupID, initialGroupID]);

    const groupQuery = useFetchGroupByID({ groupID: parseInt(groupID as string) }, !!groupID);
    const { data: group } = groupQuery ?? {};
    const groupCategoryID = group?.categoryID;

    const categoryID = useMemo(() => {
        if (formBody.groupID && groupCategoryID) {
    ...
    const postTypeOptions = useMemo(() => {
        if (groupID) {
            return originalPostTypeAsOptions() as Select.Option[];
        }
    ...
    const formLoaded =
        ...
        (groupID ? formBody.groupID && groupQuery.isSuccess && groupCategoryID : true);
                                ...
                                {formBody.groupID ? (
                                    <div>
                                        ...
                                        >
                                            <NestedSelect
                                                value={groupID}
                                                options={[{ label: group?.name ?? "Group", value: groupID }]}
                                                onChange={() => {}}
                                                required
                                                disabled
                                            />
