...
import { isPostDraftMeta, makePostDraft, mapDraftToPostFormValues } from "@vanilla/addon-vanilla/drafts/utils";
...
export function CreatePostFormAsset(props: IProps) {
    ...
    const { category, postType, taggingEnabled = false, scopedTaggingEnabled } = props;
    const userCanCreateNewTags = taggingEnabled && (hasPermission("tags.add") || isEdit) && !scopedTaggingEnabled;
    ...
    const canPostSilently = hasPermission(
        ...
        permissionCategoryID
            ? {
                  ...
                  resourceID: +permissionCategoryID,
              }
            : undefined,
    );
    ...
    const handleSubmit = async (silentPublish: boolean = false) => {
        ...
        const body = {
            ...
            ...(silentPublish && { publishedSilently: true }),
        } as ICreatePostForm;
    ...
    const manualDraftSave = async (newScheduleParams?: Partial<DraftsApi.PatchParams>) => {
        disableAutosave();
        // Determine schedule parameters based on different scenarios
        let scheduleParams: Partial<DraftsApi.PatchParams> = {};
        let publishedSilently: boolean | undefined;

        if (newScheduleParams) {
            // Extract specific values from newScheduleParams instead of spreading the entire object
            scheduleParams = {
                ...(newScheduleParams.dateScheduled && { dateScheduled: newScheduleParams.dateScheduled }),
                ...(newScheduleParams.draftStatus && { draftStatus: newScheduleParams.draftStatus }),
            };
            // Extract publishedSilently from nested structure if present
            const attributes = newScheduleParams.attributes;
            if (
                attributes &&
                "draftMeta" in attributes &&
                attributes.draftMeta &&
                isPostDraftMeta(attributes.draftMeta)
            ) {
                publishedSilently = attributes.draftMeta.publishedSilently;
            }
        } else if (isScheduledDraft && draft.dateScheduled) {
            // Preserve existing schedule for scheduled drafts
            scheduleParams = {
                dateScheduled: draft?.dateScheduled,
                draftStatus: draft?.draftStatus,
            };
        }
        // If neither condition is met, scheduleParams remains empty object
        const draftPayload = makePostDraft({
            ...formBody,
            ...scheduleParams,
    ...
    const mentionsProviderProps = {
        recordType: "category",
        recordID: selectedCategory.categoryID,
        ...(groupID && { recordType: "group", recordID: groupID }),
    };

    return (
        <MentionsProvider {...mentionsProviderProps}>
            {formLoaded ? (
                <HomeWidgetContainer
                    title={props.title}
                                ...
                                {taggingEnabled && (
                                    <div className={classes.tagsContainer}>
                                        <InputBlock
                                            ...
                                            labelNote={
                                                scopedTaggingEnabled && hasPermission("site.manage") ? (
                                                    <span>
                                                        <Translate
                                                            source="Tag creation from this field is disabled because <0>scoped tagging is enabled</0>. To create or manage tags, go to the <1/> page."
                                                            c0={(text) => <b>{text}</b>}
                                                            c1={
                                                                <SmartLink to="/settings/tagging">
                                                                    {t("Tags Settings")}
                                                                </SmartLink>
                                                            }
                                                        />
                                        ...
                                        >
                                            <TagPostUI
                                                scope={
                                                    scopedTaggingEnabled && formBody.categoryID
                                                        ? { categoryIDs: [formBody.categoryID] }
                                                        : undefined
                                                }
                                                canCreateNewTags={userCanCreateNewTags}
                                                initialTags={[
                                                    ...(formBody.tagIDs ?? []),
