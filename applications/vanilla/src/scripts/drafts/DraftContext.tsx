...
export function DraftContextProvider(props: DraftProviderProps) {
    ...
    const debouncedToSendToServer = useDebouncedInput(localDraft, 10000);
    useEffect(() => {
        void (async () => {
            try {
    ...
    const updateDraftImpl = (payload: DraftsApi.PostParams) => {
        ...
        if (!enabled || draftServerDelete.isLoading) return false;
    ...
    return (
        <DraftContext.Provider
            value={{
                ...
                removeDraft,
                draft: localDraft,
                draftLoaded: !!localDraft || draft.isSuccess,
