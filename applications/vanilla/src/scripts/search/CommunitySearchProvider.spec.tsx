...
describe("CommunitySearchProvider", () => {
    ...
    describe("autocomplete()", () => {
        ...
        it("Supports multiple search sources", async () => {
            ...
            SearchService.addSource(mockConnectedSearchSource);
            const mockAdapter = mockAPI();
            mockAdapter
                .onGet(/search/)
                .replyOnce(200, mockSearchResults, { "x-app-page-result-count": mockSearchResults.length, link: "#" });
    ...
    describe("makeSearchUrl()", () => {
        it("Should return the right url (external search url if we pass it through)", () => {
            const { result } = renderHook(() => {
                return useSearch();
            });
            result.current.searchOptionProvider = new CommunitySearchProvider();

            const defaultInternalUrl = result.current.searchOptionProvider.makeSearchUrl("test", {
                scope: "everywhere",
            });
            expect(defaultInternalUrl.includes("/search?scope=everywhere&search=test")).toBe(true);

            const urlPointingToExternalSearch = result.current.searchOptionProvider.makeSearchUrl(
                "test",
                { scope: "everywhere" },
                "https://www.google.com/search?q=%s",
            );
            expect(urlPointingToExternalSearch.includes("https://www.google.com/search?q=test")).toBe(true);
        });
    });
});
