...
interface IProps {
    currentSort?: CommentListSortOption;
    selectSort: (newValue: CommentListSortOption) => void;
    isMobile?: boolean;
}

export function CommentListSort(props: IProps) {
    ...
    const options = useMemo<ISelectBoxItem[]>(() => {
        return Object.entries(CommentListSortOption).map(([key, value]) => {
    ...
    const handleOnChange = (newValue: ISelectBoxItem) => {
        selectSort(newValue.value as CommentListSortOption);
    };
