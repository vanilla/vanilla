...
export function CommentThreadProvider(props: CommentThreadProviderProps) {
    const { children, threadDepthLimit = 3, draft, showOPTag } = props;
    ...
    const addReplyToThread = (reply: IThreadItemReply, comment: IComment) => {
        const location = `${reply.path}`.split(".");
        const partial = getThreadPartial(location, threadStructure);
        if (partial) {
            ...
            if (replyRef.current) {
                const withNewComment = addChildren({
                    path: `${reply.path}`,
                    initialThreadStructure: threadStructure,
                ...
                const withoutReply = removeReplyForm(replyRef.current, withNewComment);
                updateComment(comment.commentID, comment);
                setThreadStructure(withoutReply);
            }
        }
    };
    ...
    return (
        <CommentThreadContext.Provider
            value={{
                ...
                lastChildRefsByID,
                addLastChildRefID,
                currentReplyFormPath: replyRef.current ? replyRef.current.path : null,
                showReplyForm,
                switchReplyForm,
                addReplyToThread,
                removeReplyFromThread,
