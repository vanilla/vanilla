...
import { CommentDraftParentIDAndPath, IThreadItem } from "@vanilla/addon-vanilla/thread/@types/CommentThreadTypes";
...
export function ThreadItemComment(props: IProps) {
    ...
    const draftParentIDAndPath = getLocalStorageOrDefault(
        `${DRAFT_PARENT_ID_AND_PATH_KEY}-${discussion.discussionID}`,
        null,
        true,
    ) as CommentDraftParentIDAndPath;
    const hasActiveDraft = draftContent !== JSON.stringify(EMPTY_RICH2_BODY);
    const isDraftForThisComment = draftParentIDAndPath?.parentCommentID === props.threadItem.commentID;
    ...
    const handleReply = () => {
        ...
        if (isMobile) {
            ...
            if (!currentReplyFormRef?.current) {
