...
foreach ($this->DraftData->resultArray() as $Draft) {
    ...
    $deleteUrl =
        ...
        "?Target=" .
        urlencode("/drafts/" . $this->offset);
    $deleteText = t("Draft.Delete", "Delete");
    $deleteContent = \Gdn::themeFeatures()->useDataDrivenTheme() ? "&times;" : $deleteText;
    ?>
    <li class="Item Draft pageBox">
        <div
            class="Options"><?php echo anchor($deleteContent, $deleteUrl, "Delete", ["title" => $deleteText]); ?></div>
        <div class="ItemContent">
            <?php
            $anchorText = htmlspecialchars(val("Name", $Draft));
            if (!empty($anchorText)) {
                echo anchor(
                    wrap($anchorText, "span", ["class" => "Draft-Title", "role" => "heading", "aria-level" => "2"]),
                    $editUrl,
                    "Title DraftLink"
                );
            } elseif (!$isDiscussion) {
