...
class DockerCommand extends Console\Command\Command
{
    ...
    protected function initialize(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->useQueue = $input->getOption("queue") ?? ($this->installData()->get(self::CONF_QUEUE_CLONED) ?? false);
    ...
    private function ensureQueueCloned()
    {
        ...
        if (file_exists($queueDir)) {
            ...
            $this->logger()->title("Pulling latest version of the queue");
            $process = new Process(["git", "pull"], $queueDir);
            try {
                $this->logger()->runProcess($process);
                $this->logger()->success("Latest Queue Pulled");
            } catch (\Throwable $throwable) {
                $this->logger()->warning("Queue did not pull successfully.");
            }
