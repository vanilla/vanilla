...
(function($) {
    ...
    $.fn.popup = function(options) {
        ...
        this.mouseover(function() {
            settings.sender = this;
            if ($.popup.findTarget(settings)) $(this).addClass(settings.mouseoverClass);
        });

        this.mouseout(function() {
            settings.sender = this;
            if ($.popup.findTarget(settings)) $(this).removeClass(settings.mouseoverClass);
        });
    ...
    $.popup.init = function(settings) {
        ...
        var popupHtml = "";
        if (!settings.confirm) popupHtml = settings.popupHtml;
        else popupHtml = settings.confirmHtml;

        popupHtml = popupHtml.replace("{popup.id}", settings.popupId);

        $("body").append(popupHtml);
        if (settings.containerCssClass != "") $("#" + settings.popupId).addClass(settings.containerCssClass);
        ...
        $("#" + settings.popupId + " .Body").css({
            "max-height": pagesize[3] * 0.8,
            "overflow-y": "auto",
        });

        $(document).bind("keydown.popup", function(e) {
            if (e.keyCode == 27) $.popup.close(settings);
        });
        ...
        if (!settings.confirm) {
            ...
            var confirmHeading = settings.confirmHeading
                ? settings.confirmHeading
                : gdn.definition("ConfirmHeading", "Confirm");
            $("#" + settings.popupId + " .Content h1").text(confirmHeading);

            // 'Confirm' popup body text
            var confirmText = settings.confirmText
                ? settings.confirmText
                : gdn.definition("ConfirmText", "Are you sure you want to do that?");
            $("#" + settings.popupId + " .Content p").text(confirmText);

            $("#" + settings.popupId + " .Okay").val(gdn.definition("Okay", "Okay"));
            $("#" + settings.popupId + " .Cancel")
                .val(gdn.definition("Cancel", "Cancel"))
                .click(function() {
    ...
    $.popup.loading = function(settings) {
        ...
        if ($("#" + settings.popupId + " .Loading").length == 1) return true;

        $("#" + settings.popupId + " .Content").empty();
        $("#" + settings.popupId + " .Body")
            .children()
            .hide()
            .end()
    ...
    $.popup.settings = {
        ...
        onSave: function(settings) {
            if (settings.sender) {
                $("#" + settings.popupId + " .Button:submit")
                    .attr("disabled", true)
                    .addClass("InProgress");
            }
        },
  ...
  <div class="Overlay"> \
    ...
    $.popup.inFrame = function() {
        try {
            if (top !== self && $(parent.document).width()) return true;
        } catch (e) {}
    ...
    $.popup.getPageSize = function() {
        ...
        arrayPageSize = new Array($(doc).width(), $(doc).height(), $(win).width(), $(win).height());
