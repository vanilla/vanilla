...
class Dispatcher implements LoggerAwareInterface
{
    ...
    protected function dispatchInternal(RequestInterface $request)
    {
        ...
        if ($customPage = $this->customPageModel->lookupByRequest($request)) {
    ...
    private function handleDispatchException(\Throwable $dispatchEx, mixed $controller): ?Data
    {
        ...
        if ($dispatchEx instanceof NotFoundException) {
            $event = new DispatcherExceptionEvent($dispatchEx, Gdn::request());
