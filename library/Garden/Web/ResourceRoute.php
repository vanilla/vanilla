...
class ResourceRoute extends Route
{
    ...
    public function match(RequestInterface $request)
    {
        ...
        $extension = FileGeneratorUtils::getExtension($request);
        $outputFile =
            !empty($extension) && $request->getMethod() === "GET" && in_array($extension, self::outputFileFormats);

        // Remove any views extensions.
        if ($outputFile) {
        ...
        if ($outputFile) {
            $result->setMeta("CONTENT_TYPE", FileGeneratorUtils::getContentType($extension));
            $result->setMeta("Content-Disposition", FileGeneratorUtils::getContentDisposition($request));
        }
