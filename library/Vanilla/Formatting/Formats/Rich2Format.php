...
class Rich2Format extends BaseFormat implements ParsableDOMInterface, FormatInterface
{
    ...
    public function parse(string $content)
    {
        try {
            ...
            return new Rich2FormatParsed($content, $nodeList);
        } catch (\Throwable $e) {
            ...
            return new Rich2FormatParsed(
                ...
                new NodeList(
                    new Paragraph(
                        ["type" => "paragraph"],
                        new NodeList(new Text(["text" => self::t(self::RENDER_ERROR_MESSAGE)], new NodeList()))
                    )
                )
            );
        }
    ...
    /**
     * Ensure that parsed node lists are turned into raw contnet.
     *
     * @param Rich2FormatParsed|string $rawOrParsed
     *
     * @return string
