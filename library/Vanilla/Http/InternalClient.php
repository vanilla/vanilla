...
class InternalClient extends HttpClient
{
    ...
    public function handleErrorResponse(HttpResponse $response, $options = [])
    {
        if ($this->val("throw", $options, $this->throwExceptions)) {
            ...
            $exception = HttpException::createFromStatus($response->getStatusCode(), $message, $body ?? []);

            if ($response instanceof InternalResponse) {
                $throwable = $response->getThrowable();
                if ($throwable instanceof HttpException) {
                    $exception = $exception->withContext($throwable->getContext());
                }

                if (DebugUtils::isDebug() && $throwable instanceof \Throwable) {
                    $exception = $exception->withContext([
                        "trace" => DebugUtils::stackTraceString($throwable->getTrace()),
                    ]);
                }
            }
            throw $exception;
