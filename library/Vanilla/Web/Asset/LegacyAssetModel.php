...
class LegacyAssetModel extends Gdn_Model
{
    ...
    public function getCssFiles($themeType, $basename, $eTag, &$notFound = null, $currentTheme = null)
    {
        ...
        switch ($basename) {
            case "style":
                $this->_CssFiles = [["style.css", "dashboard", ["Sort" => -10]]];
                break;
            case "admin":
                $this->_CssFiles = [["admin.css", "dashboard", ["Sort" => -10]]];
                break;
            default:
                $this->_CssFiles = [];
        }
        ...
        foreach ($this->_CssFiles as $info) {
            ...
            if ($css) {
                ...
                [$path, $urlPath] = self::cssPath($filename, $folder, $themeType, $currentTheme);
                if ($path) {
    ...
    public static function cssPath($filename, $folder = "", $themeType = "", $currentTheme = null)
    {
        ...
        if ($folder != "dashboard") {
            $paths[] = [
                PATH_APPLICATIONS . "/dashboard/design/$filename",
                "/applications/dashboard/design/$filename",
                true,
            ]; // deprecated
        }
