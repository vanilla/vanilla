...
class CloudflareChallengeMiddleware
{
    ...
    private const CONF_CHALLENGE_AGE_CUTOFF_IN_DAYS = "premoderation.challengeAgeCutoffInDays";
    ...
    public function shouldUserReceiveChallenge(): bool
    {
        ...
        try {
            ...
            if (CurrentTimeStamp::getCurrentTimeDifference($dateRegistered) > $this->getChallengeAgeCutoffInSeconds()) {
                return false;
            }
        } catch (\Throwable $e) {
    ...
    /**
     * Return the configured length of time for a user account to be considered new.
     *
     * @return int
     */
    private function getChallengeAgeCutoffInSeconds(): int
    {
        return (int) $this->config->get(self::CONF_CHALLENGE_AGE_CUTOFF_IN_DAYS, 7) * 24 * 60 * 60;
    }
}
