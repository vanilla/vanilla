...
class WidgetBackgroundSchema extends Schema
{
     ...
     */
    public function __construct(string $description = null, bool $withXControls = true)
    {
        $colorSchema = [
            "type" => "string",
        ...
        if ($withXControls) {
            $colorSchema["x-control"] = SchemaForm::color(
                new FormOptions("Background color", "Pick a background color.", "Style Guide Default")
            );
            $imageSchema["x-control"] = SchemaForm::upload(
                new FormOptions("Background image", "Custom background image.", "Style Guide Default")
            );
        }
        parent::__construct([
            "type" => "object",
