...
class Gdn_Filecache extends Gdn_Cache
{
    ...
    {
        $required = [Gdn_Cache::CONTAINER_LOCATION];
        ...
        $cacheLocationOK = Gdn_FileSystem::checkFolderR(
            $cacheLocation,
            Gdn_FileSystem::O_CREATE | Gdn_FileSystem::O_WRITE
        );
        if (!$cacheLocationOK) {
    ...
    protected function _getKeyPath($key, $flags = 0)
    {
        ...
        if ($container === Gdn_Cache::CACHEOP_FAILURE) {
            return $this->failure(
                "Trying to fetch a container for hash '{$keyHash}' but got back CACHEOP_FAILURE instead"
            );
        }
    ...
    public function store($key, $value, $options = [])
    {
        ...
        if (
            $finalOptions[Gdn_Cache::FEATURE_COMPRESS] &&
            ($compressionMethod = $this->hasFeature(Gdn_Cache::FEATURE_COMPRESS))
        ) {
        ...
        if ($storeOp === false) {
            return $this->failure(
                "Trying to save cache value to file '{$cacheFile}' but file_put_contents returned FALSE."
            );
        }
    ...
    public function get($key, $options = [])
    {
        ...
        do {
            ...
            if (!$block) {
                ...
                $context = fgets($cache);
                [$compressed, $expires, $set] = explode("|", $context);
                ...
                if ($compressed) {
                    if ($compressionMethod = $this->hasFeature(Gdn_Cache::FEATURE_COMPRESS)) {
                        ...
                        if (!function_exists($deCompressor)) {
                            return $this->failure(
                                "Trying to decompress a value, but method '{$deCompressor}' is not available."
                            );
                        }
    ...
    {
        return $this->_exists($key) === Gdn_Cache::CACHEOP_FAILURE
            ? Gdn_Cache::CACHEOP_FAILURE
