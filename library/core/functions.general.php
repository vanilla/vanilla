...
if (!function_exists('CookieToString')) {
   /**
    * Converts a Cookie array into a valid HTTP Request string
    */
   function CookieToString($CookiesArray, $Parent = '') {
      $Cookie = '';
      $EncodeCookies = C('Garden.Cookie.Urlencode', TRUE);
      ...
      foreach($CookiesArray as $Key => $Value) {
         ...
         if(is_array($Value)) {
            $Cookie .= CookieToString($Value, $Key);
         } else {
            if(strlen($Cookie) > 0)
               $Cookie .= '; ';
