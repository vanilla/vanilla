...
import { BannerAlignment, bannerVariables } from "@library/banner/Banner.variables";
...
export const bannerClasses = useThemeCache((alternativeVariables?: ReturnType<typeof bannerVariables>) => {
    const vars = alternativeVariables ?? bannerVariables();
    const formElementVars = formElementsVariables();
    const globalVars = globalVariables();
    const isCentered = vars.options.alignment === "center";
    const borderRadius = vars.searchBar.border.radius !== undefined ? vars.searchBar.border.radius : vars.border.radius;
    ...
    const outerBackground = useThemeCache((url?: string) => {
        const overrides: Partial<IBackground> = {};
        if (url) {
            overrides.image = url;
        }

        if (vars.outerBackground.unsetBackground && (vars.outerBackground.image || overrides.image)) {
            overrides.unsetBackground = false;
        }
        ...
        return css({
            ...
            display: "block",
            ...Mixins.background({ ...vars.outerBackground, ...overrides }),
            ...mediaQueries.tabletDown(
                Mixins.background({
                    ...vars.outerBackground,
                    ...vars.outerBackground.breakpoints.mobile,
                    ...overrides,
                }),
            ),
            ...mediaQueries.mobileDown(
                Mixins.background({
                    ...vars.outerBackground,
                    ...vars.outerBackground.breakpoints.tablet,
                    ...overrides,
                }),
            ),
        });
