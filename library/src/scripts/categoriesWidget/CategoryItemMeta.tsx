...
import { ICategoryItemOptions, categoryListVariables } from "./CategoryList.variables";
...
export function CategoryItemMeta(props: ICategoryItemMetaProps) {
    ...
    const vars = categoryListVariables(categoryOptions);
    ...
    const display = vars.item.metas.display;
    const asIcons = vars.item.metas.asIcons;
    ...
    const mostRecentMeta = category.lastPost && (display?.lastPostName || display?.lastPostAuthor) && (
        <MetaItem>
            <Translate
                source="Most recent: <0/> "
                c0={
                    display?.lastPostName &&
                    category?.lastPost?.url && (
