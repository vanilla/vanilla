...
describe("Datepicker", () => {
    ...
    it("Populates and successfully edits Datepicker", async () => {
        const dateInput = await result.findByRole("date", { name: "Date Input (mm/dd/yyyy)" });
        ...
        await act(async () => {
            fireEvent.change(dateInput, { target: { value: "01/01/1901" } });
        });

        waitFor(async () => {
            await expect(dateInput).toHaveValue("01/01/1901");
        });
    });
    ...
    it("Datepicker does not receive text input", async () => {
        ...
        waitFor(async () => {
            await expect(dateInput).not.toHaveValue("dsfdsfdsfdsf");
        });
    });
});
