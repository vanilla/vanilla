    ...
    return (
        <>
            ...
            {renderViewCountAsIcon && (
                <MetaIcon icon="meta-view" aria-label={`${countViews} ${t("Views")}`} tabIndex={0}>
                    {countViews}
                </MetaIcon>
            )}
            {renderLastCommentDateAsIcon && dateLastComment && (
                <MetaIcon icon="meta-time" aria-label={`${t("Last comment")} ${dateLastComment}`} tabIndex={0}>
                    <DateTime timestamp={dateLastComment} />
                </MetaIcon>
            )}
            {renderScoreAsIcon && (
                <MetaIcon icon="meta-like" aria-label={`${score} ${t("Score")}`} tabIndex={0}>
                    {score ?? 0}
                </MetaIcon>
            )}
            {renderCommentCountAsIcon && (
                <MetaIcon icon="meta-comment" aria-label={`${countComments} ${t("Comments")}`} tabIndex={0}>
                    {countComments}
                </MetaIcon>
            )}
        </>
