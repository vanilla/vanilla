...
import { useCategory } from "@library/widgets/CategoriesWidget.hooks";
...
function usePostSettingsMutation(discussion: IDiscussion, onClose: () => void, handleSuccess?: () => void) {
    ...
    return useMutation({
        mutationFn: async ({ categoryID, postTypeID, postMeta, addRedirects }: IPostSettingsMutationParams) => {
...
function usePostSettingsFormState(discussion: IDiscussion, initialAction: "move" | "change"): IPostSettingsFormState {
    let postTypeID: string | undefined = discussion?.postTypeID ?? discussion?.type ?? "discussion";
            ...
            .custom("categoryID", {
                ...
                "x-control": {
                    ...
                    componentProps: {
                        ...
                        searchUrlFilterByDiscussionsType: true,
                    },
                },
            })
            .required();

        const isRedirect = discussion.type === "redirect";
        if (!isRedirect) {
            actionSchemaBase = actionSchemaBase
                .selectStatic(
                    "postTypeID",
                    "Post Type",
                    "Select a new post type to change this post to.",
                    postTypeOptions,
                )
                .required()
...
function useFrameTitle(discussion: IDiscussion, formValues: IPostSettingsFormValues): IFrameTitle {
    ...
    const isChangeType =
        !isRedirect &&
        "postTypeID" in formValues &&
        formValues.postTypeID !== (discussion?.postTypeID ?? discussion?.type);
...
function CustomPostTypesSettingsImpl(props: IPostSettingsImplProps) {
    ...
    // Add idea type detection logic
    const targetIsIdeaType = targetPostType?.postTypeID === "idea" || targetPostType?.parentPostTypeID === "idea";
    const currentIsIdeaType =
        discussionPostType?.postTypeID === "idea" || discussionPostType?.parentPostTypeID === "idea";
    const shouldShowIdeaWarning = targetIsIdeaType && !currentIsIdeaType;

    const ideaWarningMessage = t(
        "You're converting this post to a type that only allows upvotes and downvotes. Unsupported reactions will be removed.",
    );
    ...
    switch (currentStep) {
        case "action":
            content = (
                <>
                    {shouldShowIdeaWarning && (
                        <Message
                            className={classes.requiredWarning}
                            type={"warning"}
                            stringContents={ideaWarningMessage}
                        />
                    )}
                    <DashboardSchemaForm
                        key={stableObjectHash(actionSchema)}
                    ...
                    footer={
                        <FrameFooter justifyRight={true}>
                            <SteppedModalFooter
                                ...
                                isFinalStep={currentStep === steps[steps.length - 1]}
                                onBack={() => {
