...
export const titleBarClasses = useThemeCache((overrides?: IThemeVariables) => {
    const globalVars = globalVariables(overrides);
    const vars = titleBarVariables(overrides);
    ...
    const layoutWidget = css({
        ...((): CSSObject => {
            switch (vars.positioning.mode) {
                case "StaticTransparent":
                    return {
                        position: "absolute",
                    };
                case "StickySolid":
                case "StickyTransparent":
                    return {
                        ...sticky(),
                    };
                case "StaticSolid":
    ...
    const bg1 = css(
        {
            ...
            [`&.${swoop}`]: swoopStyles,
            opacity:
                vars.positioning.mode === TitleBarPositioning.StaticTransparent ? vars.positioning.startingOpacity : 1,
        },
        mediaQueries.compact({
