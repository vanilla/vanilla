...
export interface IContainer {
    ...
    requiresPadding?: boolean; // Use when a component wants a full mobile/desktop gutter.
...
export default function Container(props: IContainer) {
    const { tag = "div", className, requiresPadding, children } = props;
    ...
    const Tag = tag;
    return (
        <Tag className={classNames(classes.root, className)}>
            {requiresPadding && <PanelWidgetHorizontalPadding>{children}</PanelWidgetHorizontalPadding>};
            {!requiresPadding && children}
        </Tag>
    );
}
