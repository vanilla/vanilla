...
export function QuickLinks(props: IProps) {
        ...
        return [
            {
                ...
                children: linksWithCounts.map((link) => varItemToNavTreeItem(link, "quickLinks")),
            },
    ...
    }, [addComponent, linksAsNavTree, menuID, props.title]);

    // Clean up added hamburger components on unmount
    useEffect(() => {
        return () => {
            menuID && removeComponentByID(menuID);
        };
    }, []);
