...
function ActionItem(props: IProps) {
    ...
    const classes = newPostMenuFABClasses.useAsHook();
    const vars = newPostMenuVariables.useAsHook();
    const contents = (
        <>
...
export default function NewPostMenuFAB(props: NewPostMenuFABProps) {
    ...
    const aiConversationsEnabled = getMeta("featureFlags.aiConversation.Enabled", false);
    const [isAiModalVisible, setIsAiModalVisible] = useState(false);

    const aiButton = {
        label: t("Ask AI"),
        action: () => setIsAiModalVisible(true),
        type: PostTypes.BUTTON,
        id: "ask-ai",
        icon: "create-question",
        asOwnButton: false,
    };
    const items = aiConversationsEnabled ? [...originalItems, aiButton] : originalItems;
        ...
        <>
            {aiConversationsEnabled && (
                <AiConversationsApiProvider>
                    <AiChatInterfaceModal isVisible={isAiModalVisible} onClose={() => setIsAiModalVisible(false)} />
                </AiConversationsApiProvider>
            )}
