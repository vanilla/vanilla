...
describe("PostReactionsModal", () => {
    it("Renders Modal", async () => {
        ...
        waitFor(() => expect(screen.getByText("Reactions")).toBeInTheDocument());
        waitFor(() => expect(screen.getByText("No Reactions Yet")).toBeInTheDocument());
    });
    it("Renders Log", async () => {
        ...
        waitFor(() => expect(screen.getByText("Reactions")).toBeInTheDocument());
        waitFor(() => expect(screen.getByText("No Reactions Yet")).not.toBeInTheDocument());
        waitFor(() => expect(screen.getAllByText(/Test User/)[0]).toBeInTheDocument());
    });
});
