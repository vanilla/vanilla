...
export function SearchFormContextProvider(props: IProps) {
    ...
    const ALL_CONTENT_DOMAIN = useMemo<SearchDomain>(
        () =>
            new (class AllContentDomain extends SearchDomain {
                ...
                public getAllowedFields = () => ["name", "authors", "insertUserIDs"];
                ...
                public transformFormToQuery(form: ISearchForm) {
    ...
    const makeFilterForm = (form: ISearchForm): ISearchForm => {
        const allowedFields = Array.from(
            new Set([...ALLOWED_GLOBAL_SEARCH_FIELDS, ...(currentDomain.getAllowedFields(hasPermission) ?? [])]),
        );

        const searchForm = Object.fromEntries(allowedFields.map((field) => [field, form[field]])) as ISearchForm;
        return searchForm;
    };
    ...
    const updateForm = useCallback(
        (update: Partial<ISearchForm>) => {
            let updatedForm = { ...state.form, ...update };
            ...
            if ("domain" in update) {
                ...
                for (let key in updatedForm) {
                    if (!["initialized"].includes(key)) {
                        if (!nextDomainAllowedFields.includes(key)) {
