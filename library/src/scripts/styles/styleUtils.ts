...
export function variableFactory(componentName: string) {
    ...
    return function makeThemeVars<T extends object>(subElementName: string, declaredVars: T): T {
        ...
        return merge(declaredVars, normalizeVariableValues(subcomponentVars));
    };
}
 ...
 *
 * - Numbers get wrapped in `px()`.
 * - Strings starting with `#` get wrapped in `color()`;
 */
function normalizeVariableValues(variables: any) {
    if (typeof variables === "object") {
        const newObj: any = {};
        for (const [key, value] of Object.entries(variables)) {
            newObj[key] = normalizeVariableValues(value);
        }
        return newObj;
    }
