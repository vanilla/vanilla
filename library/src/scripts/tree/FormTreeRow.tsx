...
interface IProps<ItemDataType> extends IRenderItemParams<ItemDataType> {
    ...
    forceCompactDeleteAction?: boolean;
    className?: string;
    rowIconOnActiveOnly?: boolean;
}
...
export function FormTreeRow<ItemDataType>(props: IProps<ItemDataType>) {
    const { item, forceCompactDeleteAction } = props;
    ...
    const deleteAction = isDeletable && (
        <Button
            ...
            buttonType={isCompact || forceCompactDeleteAction ? ButtonTypes.ICON : ButtonTypes.TEXT}
            className={isCompact || forceCompactDeleteAction ? "" : classes.deleteHideAction}
        >
            {/*
                We display an icon on mobile in the row.
            */}
