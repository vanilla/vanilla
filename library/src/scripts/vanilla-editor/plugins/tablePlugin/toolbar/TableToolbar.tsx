...
export const TableToolbar = (props: { boundsRef?: MutableRefObject<HTMLDivElement | null | undefined> }) => {
    ...
    const { tableMeasures, multipleCellsSelected, headerType, contentAlignment, rowSizesByIndex } =
        tablesByID[tableID] ?? {};
    ...
    const toolbarPosition = useMemo(() => {
        if (tableMeasures && tableEntry) {
            ...
            const tableActualHeight = Object.values(rowSizesByIndex ?? {}).reduce(
                (acc, rowHeight) => acc + rowHeight,
                0,
            );
            ...
            const toolbarTopPosition =
                tableRect.top -
                (boundsRef?.current?.getBoundingClientRect().top || 0) +
                (tableActualHeight || tableRect.height) +
                1;
    ...
    useHotkeys(
        "ctrl+shift+k",
        (e) => {
            const tableToolbar = document.querySelector("#floatingToolbar_tableToolbar");
            if (tableToolbar) {
                const tabHandler = new TabHandler(tableToolbar);
    ...
    return (
        ...
        >
            <MenuBar className={classes.menuBar} id="floatingToolbar_tableToolbar">
                <MenuBarItem
