...
const iconData = Object.fromEntries(
    fileNames.map((fileName) => {
        ...
        svgDefs.push(
            `<symbol overflow="visible" id="${iconName}">${parsedIcon.innerHTML.replace(
...
const vanillaResourcesPath = path.resolve(__dirname, "../../resources/views/svg-symbols.html");
const iconsDistPath = path.resolve(__dirname, "dist/svg-symbols.html");

fse.writeFileSync(vanillaResourcesPath, svgDefsResult);
fse.writeFileSync(iconsDistPath, svgDefsResult);
