...
const JsonSchemaFormInstance = forwardRef(function JsonSchemaFormImpl(
    ...
    const validate = useCallback(() => {
         ...
         */
        const cleanInstance: Record<string, any> = Object.keys(instance).reduce((previous, currentKey) => {
            const currentValue = instance[currentKey];
            return {
                ...previous,
                ...(typeof currentValue !== "undefined" && { [currentKey]: currentValue }),
            };
        }, {});
    ...
    useEffect(() => {
        if (!!validation && typeof validation?.valid !== undefined) {
            onValidationStatusChange?.(validation.valid);
        }
    }, [validation]);
