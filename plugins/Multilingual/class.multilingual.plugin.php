...
class MultilingualPlugin extends Gdn_Plugin
{
    ...
    public static function enabledLocales()
    {
        ...
        if (class_exists("Locale")) {
            ...
            foreach ($localePacks as $locale) {
                if (isset(static::$overrides[$locale]["Name"])) {
                    ...
                    $locales[$locale] = mb_convert_case(
        ...
        if (!array_key_exists($defaultLocale, $locales)) {
            $locales = array_merge([$defaultLocale => $defaultName], $locales);
        }
    ...
    public function base_render_before($sender)
    {
        ...
        $sender->addAsset(
            "Head",
            "<style>.LocaleOption { padding-left: 10px; } .LocaleOptions { padding: 10px; } .Dashboard .LocaleOptions { display: none; }</style>"
        );
    }
