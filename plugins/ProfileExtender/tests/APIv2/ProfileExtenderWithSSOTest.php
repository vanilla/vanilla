...
class ProfileExtenderWithSSOTest extends SiteTestCase {
    ...
    public function testFieldsOnConnectPage(array $field) {
        ...
        if ($field['FormType'] === 'DateOfBirth') {
            $this->bessy()->getHtml('/entry/connect')->assertCssSelectorExists("select[name=\"".$fieldName.'_Day'."\"]");
            $this->bessy()->getHtml('/entry/connect')->assertCssSelectorExists("select[name=\"".$fieldName."_Year"."\"]");
            $this->bessy()->getHtml('/entry/connect')->assertCssSelectorExists("select[name=\"".$fieldName."_Month"."\"]");
        } else {
            $this->bessy()->getHtml('/entry/connect')->assertCssSelectorExists("[name=\"$fieldName\"]");
        }
    }
