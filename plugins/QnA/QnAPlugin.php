...
class QnAPlugin extends Gdn_Plugin implements LoggerAwareInterface, PsrEventHandlersInterface
{
    ...
    public function commentModel_beforeNotification_handler($sender, $args)
    {
        ...
        $isMuted = (bool) $this->database->createSql()->getCount("UserDiscussion", [
    ...
    public function discussionsApiController_normalizeOutput(
        ...
        $recordStatus = $this->recordStatusModel->getRecordStatusByID($discussion["statusID"]);
        if (!empty($recordStatus)) {
            $discussion["labelCodes"][] = $recordStatus["name"];
        }
        if (ModelUtils::isExpandOption(ModelUtils::EXPAND_CRAWL, $options["expand"] ?? [])) {
            if (!empty($discussion[self::QNA_KEY])) {
                ...
                $discussion["labelCodes"] = array_unique($discussion["labelCodes"]);
            }
    ...
    public function commentsApiController_normalizeOutput(
        ...
        $comment["answerStatusID"] = strtolower($comment["qnA"] ?? "pending");
     ...
     */
    public function updateRecordType(int $discussionID, array $discussion, string $type, ?array $postMeta = null): void
    {
        $this->discussionModel->setType($discussionID, $type, postMeta: $postMeta);
