...
class RedirectorPlugin extends Gdn_Plugin
{
    ...
    public static $Files = [
        ...
        "index.php" => [
            // smf
            "board" => ["CategoryID", "Filter" => [__CLASS__, "smfOffset"]],
            "topic" => ["DiscussionID", "Filter" => [__CLASS__, "smfOffset"]],
            "action" => ["_", "Filter" => [__CLASS__, "smfAction"]],
        ],
        "forum" => [__CLASS__, "forumFilter"],
        "forum.jspa" => [
            // jive 4; forums imported as tags
            "forumID" => "TagID",
            "start" => "Offset",
        ],
        ...
        "forums" => [
            // xenforo cateogry
            "_arg0" => ["CategoryID", "Filter" => [__CLASS__, "xenforoID"]],
            "_arg1" => ["Page", "Filter" => [__CLASS__, "getNumber"]],
        ],
        "member.php" => [
            // vBulletin user
            "u" => "UserID",
            "_arg0" => ["UserID", "Filter" => [__CLASS__, "removeID"]],
        ],
        "memberlist.php" => [
            // phpBB user
            "u" => "UserID",
        ...
        "members" => [
            // xenforo profile
            "_arg0" => ["UserID", "Filter" => [__CLASS__, "xenforoID"]],
        ],
        "messageview.aspx" => [
            // Fusetalk
        ...
        "threads" => [
            // xenforo discussion
            "_arg0" => ["DiscussionID", "Filter" => [__CLASS__, "xenforoID"]],
            "_arg1" => ["Page", "Filter" => [__CLASS__, "getNumber"]],
        ],
        ...
        "viewtopic.php" => [
            // phpBB discussion/comment
            "t" => "DiscussionID",
            "p" => "CommentID",
            "start" => "Offset",
        ],
        "topics" => [
            // get satisfaction discussion
    ...
    public function gdn_Dispatcher_NotFound_Handler()
    {
        ...
        if ($filename == "index.php") {
            ...
            if (!$get[$tryPath]) {
                ...
                unset($get[$tryPath]);
                $filename = "";
            } elseif (preg_match('#archive/index\.php$#', $path) === 1) {
                // vBulletin archive
                $filename = "archive";
            }
        }
        ...
        if (isset($Vars["DiscussionID"])) {
            ...
            if (
                ...
                $Category = $CategoryModel
                    ->getWhere(["ForeignID" => $Get["legacy"] . "-" . $Vars["CategoryID"]])
                    ->firstRow();
            } else {
    ...
    public static function forumFilter(&$get)
    {
        if (val("_arg2", $get) == "page") {
            ...
            return [
                "_arg0" => ["CategoryID", "Filter" => [__CLASS__, "removeID"]],
                "_arg1" => ["Page", "Filter" => [__CLASS__, "IPBPageNumber"]],
            ];
        }
    }
    ...
    public static function t5Filter(&$get)
    {
        ...
        } elseif (in_array(val("_arg1", $get), ["bd-p", "ct-p", "bg-p"])) {
            // Board = Category
            $result = [
                "_arg2" => ["CategoryCode", "Filter" => [__CLASS__, "lithiumCategoryCodeFilter"]],
            ];

            if (val("_arg3", $get) == "page") {
                $result["_arg4"] = "Page";
            }
        } elseif (val("_arg2", $get) == "m-p") {
            // Message => Comment
            $result = [
                "_arg3" => "CommentID",
            ];
    ...
    public static function showthreadFilter(&$get)
    {
        $data = [
            "p" => "CommentID",
            "page" => "Page",
            "_arg0" => ["DiscussionID", "Filter" => [__CLASS__, "removeID"]],
            "_arg1" => ["Page", "Filter" => [__CLASS__, "getNumber"]],
        ];

        if (isset($get["t"]) || !isset($get["p"])) {
            $data["t"] = ["DiscussionID", "Filter" => [__CLASS__, "removeID"]];
