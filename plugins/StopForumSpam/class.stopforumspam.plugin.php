...
class StopForumSpamPlugin extends Gdn_Plugin
{
    ...
    public static function check(&$data, &$options)
    {
        ...
        if ($resultString) {
            ...
            if (
                $iPFrequency >= c("Plugins.StopForumSpam.IPThreshold1", 5) ||
                $emailFrequency >= c("Plugins.StopForumSpam.EmailThreshold1", 20)
            ) {
                $isSpam = true;
            }
    ...
    public function structure()
    {
        ...
        $userID = Gdn::sql()
            ->getWhere("User", ["Name" => "StopForumSpam", "Admin" => 2])
            ->value("UserID");
    ...
    public function settingsController_stopForumSpam_create($sender, $args = [])
    {
        ...
        $conf->initialize([
            "Plugins.StopForumSpam.IPThreshold1" => [
                "Type" => "int",
                "Control" => "TextBox",
                "Default" => 5,
                "Description" => "IP addresses reported this many times will be flagged as spam.",
            ],
            "Plugins.StopForumSpam.EmailThreshold1" => [
                "Type" => "int",
                "Control" => "TextBox",
                "Default" => 20,
                "Description" => "Email addresses reported this many times will be flagged as spam.",
            ],
            "Plugins.StopForumSpam.IPThreshold2" => [
                "Type" => "int",
                "Control" => "TextBox",
                "Default" => 20,
                "Description" => "IP addresses reported this many times will be completely rejected.",
            ],
            "Plugins.StopForumSpam.EmailThreshold2" => [
                "Type" => "int",
                "Control" => "TextBox",
                "Default" => 50,
                "Description" => "Email addresses reported this many times will be completely rejected.",
            ],
        ]);
