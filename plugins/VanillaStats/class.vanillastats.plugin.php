    ...
    public function settingsController_dashboardSummaries_create($sender)
    {
        ...
        if ($this->dashboardSummariesEnabled) {
            ...
            $discussionData = $userModel->SQL
                ->select(
                    "d.DiscussionID, d.Name, d.CountBookmarks, d.CountViews, d.CountComments, d.CategoryID, d.DateInserted"
                )
                ->from("Discussion d")
            ...
            if ($daysInRange > self::USER_MAX_DAYS) {
                ...
                $userRangeWarning = sprintf(t("Data limited to %s - %s"), $fromDate, $toDate);
