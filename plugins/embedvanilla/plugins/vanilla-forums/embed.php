...
<div class="wrap">
   ...
   <h2><?php _e('&lt;Embed&gt; Vanilla', 'vanilla-forums'); ?></h2>
   <p><?php _e('Use this page to embed your Vanilla Forum into WordPress.', 'vanilla-forums'); ?></p>
	<?php vf_open_form('embed-form'); ?>
		<strong><?php _e('Forum Location in WordPress', 'vanilla-forums'); ?></strong>
		<em><?php _e('Define where to access your Vanilla Forum within WordPress.', 'vanilla-forums'); ?></em>
		<div id="edit-slug-box"><?php echo get_sample_permalink_html($post_id); ?></div>
		<?php wp_nonce_field( 'samplepermalink', 'samplepermalinknonce', false ); ?>
		<em><?php printf(__('You can further customize the page that contains your forum <a href="./post.php?post=%d&action=edit">here</a>.', 'vanilla-forums'), $post_id); ?></em>
		
		<strong><?php _e('Widget Integration', 'vanilla-forums'); ?></strong>
		<p>
			<label>
				<input type="checkbox" name="<?php echo vf_get_option_name('embed-widgets'); ?>" value="1"<?php echo $embed_widgets == '1' ? ' checked="checked"' : ''; ?> />
				<?php _e('Force your widgets to link to the embedded version of your forum instead of going to the actual forum url.', 'vanilla-forums'); ?>
			</label>
		</p>

		<strong><?php _e('Forum &lt;Embed&gt; Code', 'vanilla-forums'); ?></strong>
		<textarea id="EmbedCode" name="<?php echo vf_get_option_name('embed-code'); ?>"><?php echo $embed_code; ?></textarea>
		<em><?php _e('You can make changes to your forum embed code here (optional).', 'vanilla-forums'); ?></em>
...
function vf_configure_embed_container() {
	...
	if (!is_numeric($post_id) || $post_id <= 0 || !$post) {
		$post_id = wp_insert_post(array('post_name' => 'discussions', 'post_title' => __('Discussions', 'vanilla-forums'), 'post_type' => 'page', 'post_status' => 'publish', 'comment_status' => 'closed'));
