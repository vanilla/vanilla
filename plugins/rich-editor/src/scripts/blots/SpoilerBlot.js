...
/**
 * A blot to represent the large toggle button at the top of the spoiler.
 */
export class SpoilerButtonBlot extends Parchment.Embed {
...
export class SpoilerWrapperBlot extends WrapperBlot {
     ...
     *
     * If the deletion target would be the toggleButton, delete the whole block.
     * Also open the spoiler if something modifies its children when it is closed.
     *
     * @param {number} index - The start point for the deletion.
     * @param {number} length - The length of the deletion range.
     */
    deleteAt(index, length) {

        const [descendant] = this.descendant(SpoilerButtonBlot, index);
    ...
    /**
     * Attach the toggle button if it does not exist. Also delete the blot, if the toggle button is the only remaining child.
     */
    optimize() {
        super.optimize();

        if (!(this.children.head instanceof SpoilerButtonBlot)) {
            this.attachToggleButton();
        }

        if (this.children.length === 1 && this.children.head instanceof SpoilerButtonBlot) {
            this.remove();
        }
