...
export default class ErrorBlot extends FocusableEmbedBlot {
    ...
    public static className = "embedError";
    public static tagName = "div";

    public static create(data) {
        const jsEmbed = super.create(data) as HTMLElement;
        jsEmbed.classList.add("js-embed");
        jsEmbed.classList.remove(FOCUS_CLASS);

        const descriptionId = uniqueId("embedError-description");

        const error = document.createElement("div");
        error.classList.add("embedError-error");
        ...
        error.innerHTML = `<svg class="emberError-icon emberError-warningIcon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        ...
                        <span class="emberError-errorMessage" id="${descriptionId}">${data.message}</span>
                        <button type="button" class="closeButton js-closeEmbedError" aria-hidden="true" tabindex="-1">
                            <svg class="emberError-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        jsEmbed.appendChild(error);
        return jsEmbed;
    }

    public static value(domNode: HTMLElement) {
        const messageNode = domNode.querySelector(".embedError-errorMessage");
        return {
