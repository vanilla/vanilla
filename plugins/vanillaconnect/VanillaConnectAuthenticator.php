...
class VanillaConnectAuthenticator extends SSOAuthenticator
{
    ...
    public static function getAuthenticatorSchema(): Schema
    {
        ...
        $removeConfigurable = ["properties.sso.properties.canLinkSession", "properties.sso.properties.canSignIn"];
    ...
    protected function setAuthenticatorInfo(array $data)
    {
        ...
        $this->vanillaConnect = new VanillaConnect(
            $data["vanillaConnect"]["clientID"],
            $data["vanillaConnect"]["secret"]
        );
    }
    ...
    private function claimToSSOData($claim)
    {
        ...
        $ssoData = new SSOData($this::getType(), $this->getID(), $uniqueID, $userData, $extraData);
        $ssoData->validate();
    ...
    public function initiateAuthentication()
    {
        ...
        $url .=
            "jwt=" .
            $this->vanillaConnect->createRequestAuthJWT($this->generateNonce(), [
    ...
    protected function sso(RequestInterface $request): SSOData
    {
        ...
        if (!$this->vanillaConnect->validateResponse($query["jwt"], $claim)) {
            throw new Exception(
                "An error occurred during the JWT validation.\n" . print_r($this->vanillaConnect->getErrors(), true)
            );
        }
