...
class VanillaConnectPlugin extends Gdn_Plugin
{
    ...
    public function gdn_dispatcher_appStartup_handler()
    {
        ...
        try {
            ...
            $vanillaConnectAuthenticator = $this->authenticatorModel->getAuthenticator(
                VanillaConnectAuthenticator::getType(),
                $authenticatorID
            );
    ...
    private function connectButton(VanillaConnectAuthenticator $authenticator, $options = [])
    {
        ...
        $signInUrl =
            "/entry/vanillaconnect/signin/" .
            rawurlencode($authenticator->getID()) .
            "?target=" .
            rawurlencode($this->request->getPath());
        $registerUrl =
            "/entry/vanillaconnect/register/" .
            rawurlencode($authenticator->getID()) .
            "?target=" .
            rawurlencode($this->request->getPath());
        ...
        $result .= anchor(
            sprintf(t("Sign In with %s"), $authenticator->getName()),
            $signInUrl,
        ...
        if (!val("NoRegister", $options, false) && $registerUrl) {
            $result .=
                " " .
                anchor(
                    sprintf(t("Register with %s", "Register"), $authenticator->getName()),
                    $registerUrl,
                    "Button RegisterLink"
                );
        }
    ...
    public function entryController_vanillaConnect_create($sender, $action = "", $authenticatorID = "")
    {
        ...
        $authenticator = $this->authenticatorModel->getAuthenticator(
            VanillaConnectAuthenticator::getType(),
            $authenticatorID
        );
    ...
    public function base_getAppSettingsMenuItems_handler($sender)
    {
        ...
        $menu->addLink("connect", "VanillaConnect", "settings/vanillaconnect", "Garden.Settings.Manage", [
            "class" => "nav-jsconnect",
        ]);
    }
    ...
    protected function settings_addEdit($sender)
    {
        ...
        if ($form->authenticatedPostBack()) {
            $form->validateRule(
                ...
                "ValidateRequired",
                sprintft("%s is required.", "Client ID")
            );
            $form->validateRule(
                "vanillaConnect.clientID",
                'regex:`^[a-z0-9_-]+$`i',
                sprintft("%s must contain only letters, numbers and dashes.", "Client ID")
            );
            ...
            if (!$form->validationResults()) {
                ...
                try {
                    ...
                    $form->setValidationResults([
                        "errors" => array_column($e->getValidation()->getErrors(), "message"),
                    ]);
                }
            }
            if (!$form->validationResults()) {
        ...
        $controls = [
            ...
            "name" => [
                ...
                "Description" =>
                    t("Enter a short name that identifies this provider.") .
                    " " .
                    t("This is displayed on the sign in buttons."),
            ],
            "signInUrl" => [
                ...
                "Description" =>
                    ...
                    " " .
                    t(
                        "Use target={target} to redirect users to the page they were on or target=WhateverYouWant to redirect to a specific url."
                    ),
            ],
            "registerUrl" => [
            ...
            "signOutUrl" => [
                ...
                "Description" =>
                    t("The url that users use to sign out of your site.") .
                    " " .
                    t("Only used if the provider is set to be used as the default sign in method."),
            ],
            "sso.isTrusted" => [
                "LabelCode" => t("Trusted"),
                "Control" => "toggle",
                "Description" => t(
                    'This is a trusted provider and it can sync user\'s information, roles & permissions.'
                ),
            ],
            "sso.canAutoLinkUser" => [
                ...
                "Control" => "toggle",
                "Description" => t(
                    "This provider can link users to existing forum user by using the provided email address."
                ),
            ],
        ];
