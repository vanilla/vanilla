...
class DbaRecalculateCountsTest extends SiteTestCase
{
    ...
    public function testMultipleTables()
    {
        ...
        $this->assertEquals($totalBatch, $successIds);
    }
    ...
    public function testCategoryCounts()
    {
        ...
        $this->assertEquals(200, $response->getStatusCode());
        $body = $response->getBody();
        $randomCategoryIds = array_rand($this->categories, 3);
        $updatedCategories = $this->categoryModel->getWhere(["CategoryID" => $randomCategoryIds])->resultArray();
        $fieldsToCompare = array_keys($columns);
        foreach ($updatedCategories as $updatedCategory) {
            $categoryId = $updatedCategory["CategoryID"];
            foreach ($fieldsToCompare as $field) {
                $this->assertEquals($this->categories[$categoryId][$field], $updatedCategory[$field]);
            }
        }
    }
