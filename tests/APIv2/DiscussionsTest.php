...
class DiscussionsTest extends AbstractResourceTest
{
    ...
    /**
     * Verify cannot lookup guest user bookmaerked posts
     */
    ...
    /**
     * Verify cannot lookup guest user paerticipated posts
     */
    ...
    {
        $id = $this->testPost()["discussionID"];
        $discussionUpdates[$id] = [];
        ...
        foreach ($statusUpdateData as $updateData) {
            $newStatusDiscussion = $this->api()
                ->put("/discussions/{$id}/status", ["statusID" => $updateData[0], "statusNotes" => $updateData[1]])
                ->getBody();
            $discussionUpdates[$id][] = ["statusID" => $newStatusDiscussion["statusID"], "reason" => $updateData[1]];
        }
        /** @var RecordStatusLogModel $recordStatusLogModel */
        $recordStatusLogModel = self::container()->get("Vanilla\Dashboard\Models\RecordStatusLogModel");

        $recordStatusLogData = $recordStatusLogModel->getRecordStatusLogCount($id);
        $this->assertsame(count($discussionUpdates[$id]), $recordStatusLogData);
     ...
     * @depends testDiscussionStatusLogSave
     */
    public function testExpandDiscussionsStatus(array $discussionStatusData): void
    {
        $id = array_keys($discussionStatusData)[0];

        /** @var RecordStatusLogModel $recordStatusLogModel */
        $recordStatusLogModel = self::container()->get(RecordStatusLogModel::class);
        $rows = [$this->discussionModel->getID($id, DATASET_TYPE_ARRAY)];
        $recordStatusLogModel->expandDiscussionsStatuses($rows);

        foreach ($rows as $result) {
            $this->assertEquals($id, $result["DiscussionID"]);
            $this->assertEquals(RecordStatusModel::DISCUSSION_STATUS_REJECTED, $result["status"]["statusID"]);
            $this->assertEquals("Another some reason4", $result["status"]["reasonUpdated"]);
        }
    }

    /**
     * Test 'expandDiscussionsStatuses()' method with single discussion.
     * @param array $discussionStatusData DiscussionID to find log for
     *
     * @depends testDiscussionStatusLogSave
     */
    public function testExpandDiscussionSingleStatus(array $discussionStatusData): void
    {
        $id = array_keys($discussionStatusData)[0];

        /** @var RecordStatusLogModel $recordStatusLogModel */
        $recordStatusLogModel = self::container()->get(RecordStatusLogModel::class);
        $result = $this->discussionModel->getID($id, DATASET_TYPE_ARRAY);
        $recordStatusLogModel->expandDiscussionsStatuses($result);
