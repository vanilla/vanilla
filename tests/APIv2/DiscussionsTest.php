...
class DiscussionsTest extends AbstractResourceTest
{
    ...
    public function testGetDiscussionsByStatus(int $statusID)
    {
        ...
        // set status
        $this->api()
            ->put("/discussions/{$discussionID}/status", ["statusID" => $statusID])
            ->getBody();
        $rows = $this->api()->get($this->baseUrl, ["statusID" => [$statusID]]);
    ...
    {
        $this->expectException(ClientException::class);
        $this->api()->get($this->baseUrl, ["statusID" => [1], "statusState" => "open"]);
    }
     ...
     * @dataProvider providerTestGetDiscussionsByStatus
     */
    public function testGetDiscussionsByState(int $statusID, string $statusState, int $expectedCount)
    {
        $discussionID = self::$data["validDiscussionIDs"][0];
        // set status
        $this->api()
