...
class ModerationMessagesApiControllerTest extends AbstractResourceTest {
    ...
    protected $patchFields = ["body", "layoutViewType", "format", "isEnabled", "isDismissible", "type", "viewLocation", "includeDescendants"];
    ...
    public function testPostingToCategory() {
        ...
        $categorySpecificMessage = $this->testPost(
            $this->record(),
            [
                "recordType" => "category",
        ...
        $html = $this->bessy()->getHtml("categories/".$category["urlcode"], [], ["deliveryType" => DELIVERY_TYPE_ALL])->getInnerHtml();
    ...
    public function testMessageAppearsInSubcategory() {
        ...
        $member = $this->createUser();
        $this->getSession()->start($member["userID"]);
        $html = $this->bessy()->getHtml("categories/".$childCategory["urlcode"], [], ["deliveryType" => DELIVERY_TYPE_ALL])->getInnerHtml();
    ...
    public function testMessageDismiss() {
        ...
        $member = $this->createUser();
        $this->getSession()->start($member["userID"]);
        $html = $this->bessy()->getHtml("discussions", [], ["deliveryType" => DELIVERY_TYPE_ALL])->getInnerHtml();
        $this->assertStringContainsString($record["body"], $html);
    ...
    public function testViewingDisabledMessagesPermissionCheck() {
        ...
        $enabledMessagesCount = count($this->api()->get($this->baseUrl, ["isEnabled" => true])->getBody());
        $disabledMessagesCount = count($this->api()->get($this->baseUrl, ["isEnabled" => false])->getBody());

        // Admin should see all messages.
        $allMessagesForAdmin = $this->api()->get($this->baseUrl)->getBody();
        ...
        $member = $this->createUser();
        $this->getSession()->start($member["userID"]);
        $messagesForMember = $this->api()->get($this->baseUrl)->getBody();
