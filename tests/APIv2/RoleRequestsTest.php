...
class RoleRequestsTest extends AbstractAPIv2Test
{
    ...
    public function setUp(): void
    {
        ...
        $this->container()->call(function (RoleRequestMetaModel $metaModel, RoleRequestModel $requestModel) {
        ...
        $this->application = $this->api()
            ->post("/role-requests/applications", $req)
            ->getBody();
    ...
    public function testFilterMetasOnHasRole(): void
    {
        ...
        $this->api()->setUserID($this->memberID);
        $metas = $this->api()
            ->get("/role-requests/metas", ["expand" => "all"])
            ->getBody();
        ...
        foreach ([true, false] as $hasRole) {
            $metas = $this->api()
                ->get("/role-requests/metas", ["hasRole" => (int) $hasRole, "expand" => "all"])
                ->getBody();
    ...
    public function testPostApplication(array $meta): array
    {
        ...
        $applications = $this->api()->get("/role-requests/applications", [
    ...
    {
        $response = $this->api()->get("/role-requests/applications/{$this->application["roleRequestID"]}", [
            "expand" => true,
        ]);
    ...
    protected function assertApplicationMeta(bool $hasRole, string $status): void
    {
        try {
            ...
            $meta = $this->api()
                ->get("/role-requests/metas/application/{$this->application["roleID"]}", ["expand" => true])
                ->getBody();
            $this->assertSame($hasRole, $meta["hasRole"]);
    ...
    public function testApproveApplication(): void
    {
        ...
        $response = $this->api()->patch("/role-requests/applications/{$this->application["roleRequestID"]}", [
            "status" => RoleRequestModel::STATUS_APPROVED,
        ]);
        $this->assertSame(200, $response->getStatusCode());
    ...
    public function testDenyApplication(): array
    {
        ...
        $this->assertSame(200, $response->getStatusCode());
        $newRoles = Gdn::userModel()->getRoleIDs($this->application["userID"]);
        $this->assertEqualsCanonicalizing([$this->roleID(Bootstrap::ROLE_MEMBER)], $newRoles);
    ...
    public function testReApplyNotAllowed(): void
    {
        ...
        $application = $this->api()
            ->post("/role-requests/applications", $this->application)
            ->getBody();
    }
    ...
    public function testAllowedReApply(array $application): void
    {
        ...
        $updateAttributes = ["body" => "new body", "format" => "markdown", "email" => "new email"];
        $application["attributes"] = $updateAttributes;
        $id = $application["roleRequestID"];
        unset($application["roleRequestID"]);
        $newApplication = $this->api()
            ->post("/role-requests/applications", $application)
            ->getBody();
    ...
    public function testIndexWithMember(): void
    {
        ...
        $applications = $this->api()
            ->get("/role-requests/applications")
            ->getBody();
        foreach ($applications as $application) {
    ...
    public function testBadApprovePermission(array $meta): void
    {
        ...
        $response = $this->api()->patch("/role-requests/applications/{$request["roleRequestID"]}", [
            "status" => RoleRequestModel::STATUS_APPROVED,
        ]);
    }
    ...
    public function testExpiresFilter(): void
    {
        ...
        $this->requestModel->update(["dateExpires" => CurrentTimeStamp::getDateTime()], $where);
    ...
    private function applyToRole($override, array &$request = null): array
    {
        ...
        $response = $this->api()
            ->post("/role-requests/applications", $request)
            ->getBody();
        return $response;
    }
