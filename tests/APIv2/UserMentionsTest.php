...
class UserMentionsTest extends SiteTestCase
{
    ...
    public function testUserMentionsIndexing()
    {
        ...
        $response = $this->api()->patch("/dba/counts", ["tables" => ["comment", "discussion"]]);
    ...
    public function testLongRunnerContinue()
    {
        ...
        $response = $this->api()->patch("/dba/counts", ["tables" => ["comment", "discussion"]], [], ["throw" => false]);
        $this->assertNotNull($response["user-mentions"]["callbackPayload"]);
        $result = $this->userMentionModel->getByUser($user["userID"]);
        $this->assertEquals(1, count($result));

        // Resume and finish.
        $this->getLongRunner()->setMaxIterations(100);
        $response = $this->resumeLongRunner($response["user-mentions"]["callbackPayload"]);
        $this->assertEquals(200, $response->getStatusCode());
        $result = $this->userMentionModel->getByUser($user["userID"]);
        $this->assertEquals(4, count($result));
    }
