...
class UsersTest extends AbstractResourceTest
{
    ...
    {
        $this->runWithConfig(
            ["Feature.CustomProfileFields.Enabled" => true, "Feature.ImprovedUserProfileFields.Enabled" => true],
            function () {
    ...
    public function testPatchUserProfileFieldClearsCache()
    {
        ...
        try {
            $this->runWithUser(function () use ($user, $profileData, $userProfileData) {
                $exceptionThrown = false;
                try {
                    $response = $this->api()->patch(
                        "$this->baseUrl/{$user["userID"]}/profile-fields",
                        $userProfileData
                    );
                    $responseBody = $response->getBody();
