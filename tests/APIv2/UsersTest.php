...
class UsersTest extends AbstractResourceTest
{
    ...
    /**
     * Test that new users generated via api get the default followed category preferences.
     *
     * @return void
     */
    public function testPostUsersGetDefaultCategoryPreferences()
    {
        ...
        $this->configuration->saveToConfig(
            CategoryModel::DEFAULT_FOLLOWED_CATEGORIES_KEY,
            json_encode($categoryPreferences)
        );
        $user = $this->testPost();

        $categoryPreferences = $categoryModel->getPreferencesByCategoryID($user["userID"], $categoryA["categoryID"]);
        $this->assertNotEmpty($categoryPreferences);
        $this->assertTrue($categoryPreferences["Preferences.Follow"]);
        $this->assertTrue($categoryPreferences["Preferences.Email.NewDiscussion"]);
        $this->assertFalse($categoryPreferences["Preferences.Email.NewComment"]);
    }
    ...
    public function testDiscoveryText()
    {
        $roles = $this->roleModel->getPermissions(3);
        $users = [];
        $discoveryText = "I would be deeply honored to be a member of this community.";
        $this->runWithConfig(["Garden.Registration.Method" => "Basic"], function () use ($discoveryText, &$users) {
            for ($i = 1; $i < 3; $i++) {
                $registrationFields = $this->registrationFields([
                    "discoveryText" => $discoveryText,
                ]);
                $users[] = $this->verifyRegistration($registrationFields);
            }
        });

        // Run as admin
        foreach ($users as $userID) {
            $this->runWithAdminUser(function () use ($userID, $discoveryText) {
                // Run without any expand filter
                $user = $this->api()
                    ->get("/users/{$userID}")
                    ->getBody();
                $this->assertArrayNotHasKey("discoveryText", $user);

                // Run With expanded discoveryText
                $user = $this->api()
                    ->get("/users/{$userID}", ["expand" => "discoveryText"])
                    ->getBody();
                $this->assertArrayHasKey("discoveryText", $user);
                $this->assertEquals($discoveryText, $user["discoveryText"]);

                // Run with expand all permission
                $user = $this->api()
                    ->get("/users/{$userID}", ["expand" => ModelUtils::EXPAND_ALL])
                    ->getBody();
                $this->assertArrayHasKey("discoveryText", $user);
                $this->assertEquals($discoveryText, $user["discoveryText"]);
            });

            // Run as user without profile.view permission
            $this->runWithPermissions(
                function () use ($userID) {
                    // Run With expanded discoveryText
                    $user = $this->api()
