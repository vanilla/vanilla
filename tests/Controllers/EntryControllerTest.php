...
<option value="1">1</option>
    ...
    /**
     * Test invalid sign-in rate limit.
     */
    public function testInvalidSignInRateLimit(): void
    {
        $this->runWithConfig(
            [
                "Cache.Enabled" => true,
            ...
            function () {
                $firstExpectedErrorMessage =
                    "We're having trouble logging you in with the provided credentials. Please double-check and retry. If the issue continues, try using the Forgot Password option.";
                $this->runWithExpectedExceptionMessage($firstExpectedErrorMessage, function () {
                    $r = $this->bessy()->post("/entry/signin", [
                ...
                // The second exception message should be for a rate limit error.
                $secondExpectedErrorMessage = "Your IP is trying to log in too often. Slow down!";
                $this->runWithExpectedExceptionMessage($secondExpectedErrorMessage, function () {
                    $r = $this->bessy()->post("/entry/signin", [
                        "Email" => "notanemail@email.com",
                        "Password" => "123456",
                    ]);
