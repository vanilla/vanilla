...
class UserModelTest extends SiteTestCase
{
    ...
    public function testPasswordRequest(
        ...
        if ($captchaPluginEnabled) {
            $addonModel->enable($addonManager->lookupAddon("recaptcha"), ["force" => true]);
        } else {
            $addonModel->disable($addonManager->lookupAddon("recaptcha"), ["force" => true]);
        }

        [$passwordRequestResult, $validationResults] = $this->runWithConfig(
            ["Garden.Registration.SkipCaptcha" => false],
     ...
     *  [
     *      true/false,         // Captcha plugin enabled.
    ...
    {
        return [
            "Captcha ENABLED, checkCaptcha FALSE & existing USER." => [true, false, true, true, []],
            "Captcha ENABLED, checkCaptcha TRUE & existing USER." => [
                true,
                true,
                true,
                false,
            ...
            "Captcha DISABLED, checkCaptcha TRUE & non-existent USER." => [
                false,
                true,
                false,
                false,
            ...
            "Captcha DISABLED, checkCaptcha TRUE & existing USER." => [false, true, true, true, []],
        ];
    }
}
