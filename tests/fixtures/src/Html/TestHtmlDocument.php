...
class TestHtmlDocument extends HtmlDocument
{
    ...
    public function assertCssSelectorTextContains(string $cssSelector, string $expected): \DOMNode
    {
        ...
        if (isset($item)) {
            ...
            TestCase::fail("Could find an element matching selector '$cssSelector' with text content '$expected'");
        }
    }

    /**
     * Assert that the HTML contains a link with some url and text content.
     *
     * @param string $expectedUrl
     * @param string $expectTextContent
     * @param string $withinSelector The link must be inside of this selector.
     *
     * @return \DOMNode
     */
    public function assertContainsLink(
        string $expectedUrl,
        string $expectTextContent,
        string $withinSelector = ""
    ): \DOMNode {
        $linkSelector = "a";
        if ($withinSelector) {
            $linkSelector = "$withinSelector $linkSelector";
        }
        $links = $this->queryCssSelector($linkSelector);
        $foundLink = null;
        /** @var \DOMElement $link */
        foreach ($links as $link) {
            if ($link->getAttribute("href") !== $expectedUrl) {
                continue;
            }
            $foundLink = $link;
            if (str_contains($foundLink->textContent, $expectTextContent)) {
                break;
            }
        }

        if ($foundLink === null) {
            TestCase::fail("Could not find a link with href '$expectedUrl'");
        }

        TestCase::assertStringContainsString($expectTextContent, $foundLink->textContent);
        return $foundLink;
    }
